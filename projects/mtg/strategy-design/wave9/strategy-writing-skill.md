# Skill: Writing a per-deck strategy guide for the qwen pilot (wave 9)

## Synthesis notes (wave 9) — the CLOSING/PAUSE wave
...
- **PLAN/CHOICE MISMATCH (intent collapse).** The reply's PLAN names an action that IS a numbered
  ...
  **A distinct sibling: REPLY-PROTOCOL INTENT-COLLAPSE (answer-before-reasoning).** The "number-first,
  ...
  judged by decision-quality not win-rate (symmetric core). **This is now MULTI-SEAT (wave 9: decks
  131, 135, 140, plus a new shape from 109) and the A/B is DUE, not a watch** — the routing above is
  fixed DOCTRINE, obeyed cleanly by every witnessing seat. Three wave-9 refinements: (1) a NON-NUMERIC
