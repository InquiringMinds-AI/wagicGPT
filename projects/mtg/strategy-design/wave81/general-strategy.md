...
# WAVE-81 ADOPTED — R332-R333

**This section is the only change from `wave62/general-strategy.md`** (waves 63-80 wrote no
edition; the prefix above is byte-verbatim and was verified with `cmp -n 61905`). Wave-81 corpus
`~/.Wagic/ai/gpt/selfplay-runs/matchups-20260917-191141-final`, binary
`archives/wagic-6e0c51238-w81step1` (master 6e0c51238), 21 games, 42 seat logs, all `gameend`,
0 HUNG, 0 crashed; 2,122 decision windows, **one** heuristic fallback (0.05%), 100% native
reasoning. Pool 123, 125, 130, 146, 152, 162 and **50** (Erosion, new, guideless first seating).
Adoption/rejection reasons and the merged engine/interface/card list: `wave81/synthesis-notes.md`.
Two proposals filed, two adopted.

## NEW — RENDER, HIGH

**R332. The seat's OWN library line carries no deck-out clause, and no cast row prices the mill
the cast itself triggers — so a pilot reads the opponent's library as a win condition and its own
as nothing at all.** *(deck125 P-G1, with its own items H1 and H2.)* The prompt prints
`Opponent library: N cards - DECK-OUT IS IN RANGE: a player who must draw from an empty library
LOSES …` and, four lines later, a bare `Your library: N cards`. Corpus-wide over all 42 seat logs
and 2,122 windows, `DECK-OUT IS IN RANGE` renders on the **opponent line only** — 15 windows — and
on the seat's own line **0 times**; the only annotation the own-library line ever carries is the
search/reveal parenthetical (6 windows).

**Repro** — `1789690337-ai_baka_deck125-0x55f86ec66780-vs-ai_baka_deck50.jsonl` **seq305**, which
prints `Opponent library: 1 cards - DECK-OUT IS IN RANGE: …` above a bare `Your library: 11 cards`.
deck125 was at **59 life** against an opponent on **7** with one card left in their library, and
lost at turn 42 by drawing from its own empty library. The second half of the same failure is the
cast row's forecast: with four Memory Erosions on their battlefield (every spell the seat casts
mills it eight), **seq282** option 12 read `X = 12 … {library: this draws 12 of your 25 library
cards - 13 left; 6 draws are already owed that you cannot decline, and the 13 left still pays
them}` — and the library at **seq283** was **5**, not 13. Same shape at seq301 (`Cast Cancel` taken
at library 30, library 22 at seq302).

**Ask.**
1. Render the `DECK-OUT IS IN RANGE` clause on the seat's OWN library line under the same
   predicate that fires it on the opponent's, so the loss condition and the win condition are
   written in the same grammar on the same screen.
2. Give the cast/X row's `{library: …}` brace a cast-price clause of the `DRAW PRICE:` kind: when a
   permanent on either battlefield mills the caster on each cast, the row's own count must carry
   it (`every spell you cast mills you N` / the forecast net of that N). The row today counts the
   draws it makes and nothing the cast costs, which is why the promised `13 left` was 5.

**Scope, and the guide-side residual.** The paragraph deck125 proposed for the pilot —
*"Your library is a life total you cannot see fall … a card of theirs that says an opponent who
casts a spell mills that player prices EVERY cast of yours, and no row's own count includes it"* —
is adopted **as this render ask, not as standing text**: the seat itself ranked the render fix
above the text and named the retirement (with 1 shipped, the first two sentences become a
derivation the render performs; with 2 shipped, the last one retires too). Deck50's arrival makes
this shape reachable against every seat in the pool, so it is filed general rather than at deck125.
deck50's own new guide (`wave81/deck50/strategy.txt`) teaches the mirror fact — the two clocks as a
race — from the milling side, and is consistent with this.

## CORE-PROMPT

**R333. The counter test must key to the two facts the screen already prints, not to one deck's
cast-order entry — a row that spends the whole turn's mana on ONE object into open counter mana is
the wrong row on any board.** *(deck146 P-146-1.)* deck146's guide carries a COUNTER TEST that
works: at `1789690316-ai_baka_deck146-0x555bfc3f8960-vs-ai_baka_deck125.jsonl` **seq23** the seat
deferred Lolth by name, citing it. But the test is filed inside cast-order entry 2 and therefore
reaches only that deck's two planeswalkers; over the following eighteen turns the same seat walked
**eight** other spells into the same open mana, one per turn — Nadaar (seq74), Acererak (seq95,
seq98), Emeria's Call hard-cast for `{4}{w}{w}{w}` into 15 untapped `{u}{w}` sources while its own
land row sat on the same menu (seq128), Silverquill Command (seq130), Triumphant Adventurer
(seq151), Silverquill Silencer (seq189), Soul Shatter (seq227). `opp_countered_spells` for that
game is **12**; `counter_intel_rendered` is **1319**, so the intel was on the screen at essentially
every window. The seat lost that game at **-11 to 63** over 47 turns.

**Ask.** State the test once, in the shared prompt, in terms of the two facts the render already
supplies — the counter named on the opponent's line as one they can still cast, and
`Their untapped sources` covering its printed cost: while both hold, a window whose chosen row
would spend your whole turn's mana on ONE object is the wrong window — take the cheapest row that
leaves your own sources up, or the land row, and put the expensive object on a later window where a
second row also gets taken.

**Scope, and what it is NOT.** Not "hold every threat": the seat that holds everything is the seat
that discards it, and the same game shows that too — deck146 binned both copies of Lolth at cleanup
(seq48, seq60) and then spent 24 turns with no win condition. The rule fires only on the row that
spends the whole turn on one object, which is the shape every countered spell in that game had, and
it generalises a test this corpus shows WORKING at seq23 rather than inventing one. Filed
CORE-PROMPT because it is owned by no deck: the pool's counterspell deck (125) plays against six
others, and deck146's entry-2 scoping is an accident of where the rule was written, not a fact
about Lolth.

## MEASUREMENT — added to the wave-57/58/62 list

15. **R332**: windows whose own `Your library: N cards` line is bare while N is at or under the
    deck-out predicate the opponent's line uses — **0 of N** (wave 81: 2,122 of 2,122 bare, 15
    opponent-side renders); and cast/X rows whose `{library: …}` count differs from the library
    measured at the next window while a mill-on-cast permanent is on either battlefield line —
    **0 of N** (wave 81: 2 known repros, no census run).
16. **R333**: spells cast into a board printing both a castable named counter and
    `Their untapped sources` covering it, where the chosen row spent every untapped source the seat
    had — **0 of N** (wave 81: 8 in one game, `opp_countered_spells` 12).
