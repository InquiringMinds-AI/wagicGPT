# Skill: Writing a per-deck strategy guide for the qwen pilot (wave 10)

## Synthesis notes (wave 10) — the RESUME/VERIFICATION wave (first corpus under answer-after-plan)
...
- **PLAN/CHOICE MISMATCH (intent collapse).** The reply's PLAN names an action that IS a numbered
  ...
  **A distinct sibling: REPLY-PROTOCOL INTENT-COLLAPSE (answer-before-reasoning).** The "number-first,
  ...
  take the wrong number in all 4 deck140 cases. A single-seat sub-variant (deck110 P3): the plan fixates on
  an UNLISTED card and emits a poorly-mapped fallback that contradicts its own sub-conclusion — WATCH,
  promote with a 2nd seat. **Two metric cautions across the protocol boundary:** (i) the old head-first
