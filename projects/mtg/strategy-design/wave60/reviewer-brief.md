# Wave-60 step-3 brief (lean; the owner's loop definition is wave58/loop-as-envisioned.md; wave 58 adopted 14 of 14 proposals and the owner flagged that as the busywork pattern; wave 59 adopted 0 of 1 — REJECT is the expected verdict for anything not paid for by a decision the corpus shows going wrong)
...
wagic; write only under projects/mtg/strategy-design/wave60/. Never write mechanisms as the
owner's positions. Report what you did NOT check.

CORPUS (wave 60, binary = master 614f8452e = wave-59 close + wave-60 step 1: lanes L (B1 win fold
subtracts cast life cost + stack, B2 ON THE STACK damage total, B11 ATTACK TOTAL), M (B4 connect-timeout
fraction + transport_error, B3 PUT duplicate index re-ask, B13 plan-echo bound + decline counter), N (B5
discard-punisher lens + cleanup price, B6 loop caution + LOOP HALF PENDING), O (B7 sweeper rosters, B8
Teferi focus + tap/untap tags, B10 forced-sacrifice ask, B12 venture source), P (B9 exile hostage rows, B14
reveal option two arms), and Q (the nine Codex-review fixes: summed naming punishers, reveal-gate ownership,
cleanup price as a ceiling, ATTACK TOTAL infect/double-strike, tap tag reads CANBLOCKTAPTED/DOESNOTUNTAP,
indestructible survivors, printed hostage P/T, plan marker survives trim, `bad_reply` class); read
wave60/known-bugs.md, wave60/lane-{L,M,N,O,P,Q}.md and wave60/codex-review.md for what changed and each
lane's falsifiable predictions): /home/magi/.Wagic/ai/gpt/selfplay-runs/matchups-20260904-171140/ (21 games, 21 natural, 0 crashed, 0 hung).
...
as guide effect.
Output: ONE file, wave60/deck<N>/review.md: (1) game-by-game outcome + the deciding decisions with
...
success verdict, state why) or EDIT — if EDIT, also write wave60/deck<N>/strategy.txt (the full
...
your evidence supports them: general-guide changes (wave60/deck<N>/general-proposals.md) and
strategy-writing-skill changes (wave60/deck<N>/skill-proposals.md; current skill edition =
...
You read the whole corpus for the engine's sake, not the guides'. Deliverable wave60/engine-seat.md:
...
wave60/lategame-specimen.txt with a 3-line header (file, seq, turn, why this one). Final
...
Read all proposals + engine-seat.md. Decide each proposal: ADOPT (edit wave60/general-strategy.md,
a copy of wave59's, or append a numbered amendment to wave60/strategy-writing-skill.md, a
byte-verbatim copy of wave59's with amendments appended — verify the prefix with cmp) or
REJECT with one line of reasons. Write wave60/synthesis-notes.md. If a deck agent's guide edit
