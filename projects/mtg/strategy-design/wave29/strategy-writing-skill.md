# Skill: Writing a per-deck strategy guide for the qwen pilot (wave 29)
...
- [ ] If the deck's engine implements an ALTERNATE WIN CONDITION (life/poison/mill-self/permanent
      ...
      bonus" + a bridge to it — never left to option text (wave-29 HL6). For a resource the model treats
      as a cushion but is actually the engine (lifegain-matters etc.), the guide leads with the
      resource-reframe and per-card thresholds.
- [ ] A Step-0 construction loss is decomposed CONSTRUCTION-TERMINAL (guide validated, record cannot
      ...
      then route to roster owner) — wave-29 HL5. Do not route a ceilinged seat prematurely.
