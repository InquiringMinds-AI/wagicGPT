# Skill: Writing a per-deck strategy guide for the qwen pilot (wave 7)

## Synthesis notes (wave 7)
...
2. **The general system prompt** — `bin/Res/ai/gpt/system_prompt.txt`. EVERY deck already
   ...
   `hand -> stack` anchor. Its defect is FIXED at source (353f1a150) and wave 7 is the SECOND
   consecutive clean no-op corpus (0 cast no-ops across all 7 seats, 2 waves), so its removal condition
   is MET and the anchor is SCHEDULED for a deliberate A/B removal in wave-8 (with a control arm — it
   stays in the shipped prompt until then; exact old→new in wave-7 `core-prompt-revision.md`). Do NOT
...
- **Retiring the `hand -> stack` cast-happened anchor as a WAVE EDIT** — REJECTED as a wave edit;
  SCHEDULED as a deliberate A/B for wave-8 (condition met: 2 consecutive clean no-op corpora). Removal
  of a deck-agnostic core line is always an A/B with a control arm, never a silent wave edit.
