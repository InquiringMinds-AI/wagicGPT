# Wave-46 seat-reviewer brief (common to all seats)
...
1. projects/mtg/strategy-design/wave46/review-carry.md  (new-baseline discipline + the
   prediction sets you must adjudicate — PASS / FAIL / UNTESTED; UNTESTED is a real verdict
   when no window arose; never infer).
2. projects/mtg/strategy-design/wave45/strategy-writing-skill.md (the CURRENT skill edition, amendments 1-67:
...
3. Your seats' wave-45 file(s) in projects/mtg/strategy-design/wave45/seats/ (rung state,
   open predictions), and the live guides projects/mtg/bin/Res/ai/baka/deckN_strategy.txt.

CORPUS: ~/.Wagic/ai/gpt/selfplay-runs/matchups-20260826-155842/ — one JSONL per SEAT
...
- projects/mtg/strategy-design/wave46/seats/seat-<ids>.md — same shape as the wave-45 seat
...
- projects/mtg/strategy-design/wave46/deck<N>/strategy.txt — the FULL revised guide per deck
...
- projects/mtg/strategy-design/wave46/deck<N>/skill.md — proposals for the strategy-writing
...
WAVE-46 SPECIFICS (owner doctrine 2026-08-26): YOU own your decks' guides — every guide change
is your decision from corpus evidence; never route a deck-content question to the owner (no
'OWNER QUESTION' sections). The core loop's purpose is engine mechanical correctness and the
LLM UX (legality enforced structurally without constraining choice; information and choices
presented to maximize decision quality; efficient inference) — your engine/render items feed
that; guide edits are the cheap side quest. Adjudicate every render prediction by the
emitter's actual string. This corpus is the FIRST on the wave-45 reviewer guides + boundary
pass, and the first carrying lanes F/G/H/I and lane E (see wave46/review-carry.md for each
lane's falsifiable prediction). Owner rulings that stand: X menu largest-first; history lines
owner-prefixed.
