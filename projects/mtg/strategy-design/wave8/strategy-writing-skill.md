# Skill: Writing a per-deck strategy guide for the qwen pilot (wave 8)

## Synthesis notes (wave 8)
...
- **PLAN/CHOICE MISMATCH (intent collapse).** The reply's PLAN names an action that IS a numbered
  ...
  **A distinct sibling: REPLY-PROTOCOL INTENT-COLLAPSE (answer-before-reasoning).** The "number-first,
  ...
  judged by decision-quality not win-rate (symmetric core). Single instance -> arm the metric (recorded
  choice == the reply's own concluded option?) and WATCH; do not change the protocol on one datum.
...
- **The ELSE-branch of a priority rule needs its own IMPERATIVE, not a permission — and it must NAME
  ...
  applied to a priority rule's else.
