...
## 2026-09-06 22:15 — WAVE-70 STEP ONE CLOSED + FIRST THINKING-ON CORPUS LAUNCHED. Gate 1 on 71b1e22be green (1279/2-known lifeline+merrow, 67 AI/0, 0 timeouts, PARSETEST 5283/0). Astra review (wave70/codex-review.md, 12 findings, all CONFIRMED by BN): prose still vetoed blocks / manufactured declarations / clamped repeats via stop numbers scraped from prose; `max_reply_tokens` at reasoning_budget=0 starved reasoning; regime gate false-FAIL (forced-close, engine-only records) and false-PASS (5 pooled records, hidden reasoning under off); plan steps advanced on parse not execution; commas split card names; deck123/130 templates still ledgers. BN b1969dacd -> 00dcf99f9 (clean): prose readers DELETED (truncatedBlockCommitmentAbandoned, salvageProseAttackers/Blocks, protocolLinesOnly() for the stop scans, planArguesAgainstRow reads the PLAN line), reasoning never 0 under thinking-on (kDefaultReasoningBudget fallback), gate fixed per seat log + forced-close + WAIT + hidden-reasoning FAIL, step consumption in writeTransLog on execution, step grammar `;`/newline/`then`/`.!?`+Upper (0 boundaries inside card names over 1,582 w69 PLAN lines vs 13 on base), guides deck123/130 templates one line (staged + live). Gate 2 on 00dcf99f9 green (same numbers, PARSETEST 5302/0). Guides installed e68010dfa (6 files 19,772-19,999 B, A332 0). Guard now scans bin/Res/ai/baka at link time (14c82df37). Archive `archives/wagic-14c82df37-w70step1`; pushed; lane worktrees removed. CORPUS `matchups-20260906-221214`: `--thinking on`, 21 games -j 21, WAGIC_GPT_TIMEOUT=900, NO WAGIC_GPT_MAXTOKENS (answer ceiling under BK), unit w70-corpus 12G, script ~/.gatelogs/w70-corpus-launch.sh. Open question to owner: the golden protocol's PLAN exemplar uses bare commas while the step grammar splits on `;`/then — his surface, untouched.

## 2026-09-06 22:50 — FIRST THINKING-ON CORPUS matchups-20260906-221214 KILLED BY A FALSE REGIME FAIL after PASS. The gate PASSED (210 records, all reasoning, all two_line, 42 seats); the next sweep gates only NEW seat logs, found none, returned WAIT "no gateable records yet", and the 30-min no-records deadline turned that into FAIL + kill (exactly BK's named risk). Fix: the deadline applies only before REGIME-GATE-PASSED exists (harness only; regime-gate.py untouched). By-hand early check before the kill: 198/198 model records reasoning_chars>0 (min 1,645, median ~4,184), thinking=on, every reply exactly PLAN+action. The killed run's logs are NOT a corpus. Relaunched as a fresh run.

## 2026-09-07 07:05 — FIRST THINKING-ON CORPUS DONE: matchups-20260906-224849 (~8h wall, -j 21): 20/21 natural, 0 timeouts, 0 crashes, 1 HUNG (deck152 vs deck126, game-152v126-1788752967: 152 seat seq 55 t15 M1 ask choice 3 with opp at 1 life, then NO tick for 3641 s -> harness stopped that game only; rerun launched matchups-20260907-065922, unit w70-rerun, done-file W70_RERUN_DONE). 2,119 model decisions, 100% with native reasoning, 9 fallbacks, 0 truncations, 0 action_before_plan, 103 (4.9%) records with off_protocol_bytes>0, latency p50 80.7 s / p95 185.6 s. Wins: 152 4/6, 126 2/6, 162 2/6, 123 1/6, 130 1/6. Step 3 launched: deck seats 146/125/162/123/130 + engine seat (item 0 = the hang hypothesis, then the compensation census + the 103 off-protocol records); deck 152 and 126 seats wait for the rerun. Brief: wave70/reviewer-brief.md (7d1cf5935).

## 2026-09-07 — WAVE-70 STEP 3 (5 decks + engine seat) DONE; known-bugs written (wave70/known-bugs.md, ff4cf0c7f). THE CENSUS ANSWER: every wave 66-69 in-band-reasoning mechanism is DEAD under reasoning-on (plan_choice_conflict 25->0, prose overrun 1,588->2, reply_truncated 58->0, reversal/index/repeat re-asks 0); fallbacks 1.93%->0.42%; 94.6% replies exactly PLAN+action, ZERO prose (the 103 off-protocol = unlabelled plan line, 7 of them `YOUR PLAN:` = the heading transcribed); latency p50 10.6->80.7 s (7.6x), 900 s wall holds 2x. Revert list = known-bugs §R (R1-R9). THE HANG = L1 (deterministic: MDFC face-menu decline not sticky -> Baka re-proposes -> mAskCache silent replay, no record -> pass floor never fires; 2.58M "not passing" lines). New HIGH engine: L3 rowSaysNoOp matches Tragic Slip's "does not apply"; L4 castable instant has no cast row at priority windows (Starstorm 23/23); L5 two-step cast reverses to cycling; L6 Tribute to Hunger double-credit; L7 animating lands in no count; L8 reserve verdict vs a hand a pending Puzzle Box trigger bottoms. Guides 146/125/162/123/130 EDIT verdicts installed (<=19,998 B, A332 0, guard OK). No proposals -> no synthesis. Record field is `reasoning` (NOT reasoning_content) — skill/memory/brief corrected. Rerun 152v126 (matchups-20260907-065922) still running; deck 152/126 seats after it. Lategame specimen: wave70/lategame-specimen.txt (alert-only).

## 2026-09-07 08:15 — WAVE-71 STEP ONE MERGED (base 77d82bbe6 = w70 close + deck152/126 guides 9d3817168 + brief): BP ecb17f4f3 -> 6779ae56b (clean; hang: declined face latched at DecisionManager::applyMenuChoice, pass-floor NO-PROGRESS arm (200 ticks), mAskCache refuses after 64 replays, ask_replay records + gameend counters, harness fast watchdog = translog silent 300 s AND stderr +64 MB; fixtures ai_mdfc_decline_{livelock,sticky}_w71bp RED on base). BQ 2dfcb3c49 -> 3e2c7a0fa ((ci) clean; L3 rowSaysNoOp: victim tag defeats no-op, phrase only at paren depth 0 outside quotes, no hypothetical scope — 45 of 2,914 rendered rows flip TRUE->FALSE; L4 own-turn instant windows enter the response branch of computeActions (SHARED path — fixture ai_instant_own_end_step_w71bq RED on base); L5 cast-mode commitment note; L12 reveal ask states its scope). BO 2974f3627 -> 8857750dc (ctor init-list conflict: six BO-deleted members re-added by the merge, removed; revert list R1-R9: 10 functions, 7 members, 5 fields, ~25 notes, 7 seams of re-asks, ~87 pins; answer ceilings re-fit p99.9x2/3.15 floor 256: ask/priority 288, attackers/blockers/discard/reveal 256, bottom 512 UNTESTED; L9 markers cleared per consume (19 stamps vs 4 closes); L10 plan_line_missing + plan_steps_done/plan_step_count). BR abfe27f75 -> e94565edc ((ci) stray brace at the L9/L7 PARSETEST seam; L6 ENGINE: StackAbility::resolve stamps AADynamic::resolvingFromStackAbility, toughnesslifegain refuses payment as a stack object once the victim is gone — fixture tribute_to_hunger_double_w71 RED on base; L7 manlands stated beside counts; L8 pending-hand line; L11 YOUR PLAN heading now a quotation; L13-L17, L18a, L19; L18c parser refusals deliberately NOT done — HOLD PRIORITY row head is label-shaped, docketed). PARSETEST 5314/0 after all four (5302 +7 +21 -75 +59). Gate 1 unit w71-gate1 running; Astra review launched (~/.gatelogs/w71-review-prompt.md, diff 77d82bbe6..e94565edc).

## 2026-09-07 09:00 — WAVE-71 STEP ONE CLOSED. Astra review (wave71/codex-review.md, 9 findings, all CONFIRMED by BS fd1c5f411 -> b31f66cf3): L6 stamp narrowed to the edict shape (Proper Burial fixture), Tribute second copy re-chooses a remaining creature at AADynamic::resolve (NOT StackAbility::resolve — placeholder target there), land latch scoped to a board fingerprint + proved land drop, cache refusal clears both caches + ask_replay records to an askreplay/ sidecar (watchdog ignores), answer ceiling = max(seam cap, cardinality floor 16+items*bytes+256 B) + one prose-free doubled-allowance re-ask on finish_reason=length at declaration seams, distinct-index re-ask restored as one labelled-line re-ask (no auto-fill), Puzzle Box hand from the pending payload's target zone, life-neutral negative only with no life-affecting primitive, rowSaysNoOp branch-aware. Gate 2 on b31f66cf3: 1280/2 (lifeline + intrepid flake), 73 AI/0, 0 timeouts, PARSETEST 5330/0. Archive archives/wagic-b31f66cf3-w71step1; pushed; worktrees removed. CORPUS w71 launched: unit w71-corpus, --thinking on, script ~/.gatelogs/w71-corpus-launch.sh, done-file W71_CORPUS_DONE. Predictions to adjudicate: 0 HUNG (BP), reply_truncated 0 at 288/256 ceilings (BO tripwire), noop_row false fires 0 (BQ), Tribute/lifegain correct (BR/BS), fallback rate <= 0.42%.

## 2026-09-07 13:10 — WAVE-71 CORPUS DONE: matchups-20260907-085638 (~4.2h, -j 21): 21/21 natural, 0 HUNG (BP prediction PASS at corpus level), 0 timeouts/crashes. 2,159 decisions, 100% reasoning, 4 fallbacks (all noop_row_zero_reask — genuineness pending the engine seat), 0 reply_truncated at 288/256 (BO tripwire clean), 0 action_before_plan, off_protocol 83 (3.8%), plan_line_missing 90 (4.2%, newly visible), latency p50 79.3 s / p95 188.6 s (unchanged). askreplay/ sidecar EMPTY — engine seat to say whether zero re-serves or a wrong path. Wins: 146 5/6, 162 4/6, 152/126/125 3/6, 130 2/6, 123 1/6. Step 3 launched: 7 deck seats + engine seat (regression watch on the shared-path changes). Brief df0613fe5.

## 2026-09-07 15:30 — WAVE-71 STEP 3 DONE; WAVE-72 CUT. Regression watch CLEAN (decisions/turn 2.25->2.40, 0 hangs, stderr 1.23 GB->557 KB, no ping-pong, no forced passes, 0 refusals). Predictions: BO 5/5, BQ L3/L5/L12 PASS + L4 substance-PASS (priority windows carry no cast row — the sibling ask does), BR L8 + BS F7 FAIL (Puzzle Box predicate), BS F1-F6 UNTESTED. 3 of 4 fallbacks FALSE (prospective zero on a PERMANENT — new no-op class). askreplay sidecar lives at ~/.Wagic/ai/gpt/logs/askreplay/ (725 records), harness never harvests it. Deck seats: 146 5/6 (wave-70 edit PAID), 162 4/6, 152/126/125 3/6, 130 2/6, 123 1/6 (mana). All seven wave-71 guide EDITs installed 03dd75f8e (the two guides quoting the "YOUR PLAN" heading had that quote removed — deck125 measured it doubling the mislabel rate). wave71/known-bugs.md §M (M1-M23) -> wave72/lane-brief.md: BT (M1 harvest, M2 prospective-zero no-op + named re-ask, M10 stop-reached rows, M13, M14 own-turn budget, M22 window-scoped refusal), BU (M4/M5 opponent-turn response windows — SHARED PATH), BV (M6-M9, M12, M16-M18), BW (M3 Puzzle Box fixture, M11, M15 report, M19-M21, M23). Base 0e4e6b9b8; worktrees w72-{BT,BU,BV,BW}.

## 2026-09-07 16:05 — WAVE-72 STEP ONE MERGED (base 0e4e6b9b8): BT aaec1afb6 -> 565cc5218 (clean; M1 harvest + selftest, M2 prospective-zero rescue + named re-ask, M10 stop-reached window collapse (`stop_reached_windows_skipped`), M13 answered-tag, M14 REFUTED (instrument `own_turn_windows_skipped` only), M22 per-window refusal map — max real run was 11 not 38). BU 62ebc0059 -> bdfe947e9 (clean; ONE call per window: casting ask first else priority ask; a HOLD closes the WINDOW (sibling seam suppressed over same turn/phase/board — `sibling_window_asks_skipped`); "Cast nothing" closes casting only; M5 REFUTED — three opponent-turn fixtures GREEN on base; ROOT CAUSE for deck130's 4 windows: an abandoned {X} announcement destroys the payment and leaves lands tapped (`MTGPutInPlayRule: cast of … dropped after its X was announced`) — DOCKETED wave 73, not fixed (refund needs a re-offer guard)). BV 723a43156 -> daf4605e9 ((ci) stray brace; M6 third hold spelling for activation menus, M7 token names removed at the ENGINE (CR 201.3) + copies-not-public + reorder with inverse permutation (UNTESTED live), M8 perspective frame (mapping not rewrite), M9 land drop folded into reserve + consequence-first verdict + upkeep->draw carry REFUTED (hand/library change the key) -> prompt note only, M12, M16 (sibling loyalty rows were the truncated ones), M17, M18). BW cea507626 -> 32ab3855c (PARSETEST 1 fail: BV's activation spelling still had the colon BW removed — head unified to "Hold priority -"; M3 ROOT CAUSE one layer down: StackAbility never assigns Interruptible::source → fixture with a Box on each side RED on base; M11 life-race clause; M15 MEASURED: 89 of 90 unlabelled plans sit under "Write your PLAN: line first" instructions — rate tracks ACTION-clause complexity, not label capping; M19 LOOP COMPLETE header; M20; M21 decline row states scope; M23 tax + de-coloned hold head; dungeon header REFUTED (renders 20/20; venture-step sentence 0/20 residual)). PARSETEST 5457/0. Gate 1 unit w72-gate1 + Astra review launched (diff 0e4e6b9b8..32ab3855c).

## 2026-09-07 17:15 — WAVE-72 REVIEW + BX. Gate 1 first run (multi-threaded — the gate scripts inherited from w69 lacked WAGIC_TESTSUITE_THREADS=1) flaked ai_instant_own_end_step_w71bq (another game's commands interleaved in its log; 5/5 solo); rerun THREADS=1 on the same binary 1281/0, 76 AI/0, PARSETEST 5457/0. GATE SCRIPTS NOW RUN THREADS=1 (w72-gate1/gate2 scripts; sed future gates from these, not from w69's). Astra review (wave72/codex-review.md, 9 findings, all CONFIRMED by BX 1d7f7b02c -> ceb9b8de0): F1 hold covers the sibling only if its acting rows are a subset; F2 mStatedStopTurn dated by THIS reply's PLAN stop + ownership (`their stop=`); F3 mCastDecisionOpen RAII + stamp after validation; F4 token filter by card-name lookup (CR 201.4); F5 "spent" claim removed; F6 fallback stays in engine index space; F7 land drop counts only if it makes mana (fetchlands/Dark Depths/Maze rejected); F8 conditional unblocked claim naming the animator/token maker; F9 LOOP COMPLETE gated on lifeLoopProvenWin. PARSETEST 5487/0. Gate 2 (make -B + THREADS=1) running; corpus script ~/.gatelogs/w72-corpus-launch.sh ready.

## 2026-09-07 16:40 — WAVE-72 STEP ONE CLOSED: gate 2 on ceb9b8de0 (make -B, THREADS=1) 1281/0, 76 AI/0, 0 timeouts, PARSETEST 5487/0. Archive archives/wagic-ba6af25f1-w72step1; pushed; worktrees removed. CORPUS w72 launched: unit w72-corpus, --thinking on, script ~/.gatelogs/w72-corpus-launch.sh, done-file W72_CORPUS_DONE. Predictions to adjudicate: sibling_window_asks_skipped ≈45% of same-window pairs (BU, now narrowed by BX F1 subset rule), stop_reached_windows_skipped>0 with no legal row lost (BT/BX F2), noop_row false fires 0 (BT M2), askreplay/ harvested into the run dir (BT M1), name-menu token names absent (BV/BX F4), Puzzle Box line fires on the live shape (BW M3), 0 hung, decisions/turn ≤ 2.40.

## 2026-09-07 21:20 — WAVE-72 CORPUS DONE: matchups-20260907-163626 (~4.7h): 21/21 natural, 0 hung. 1,910 decisions (w71 2,159 — own_turn_windows_skipped 718 is the new M14 instrument; engine seat to say whether any were lost legal windows), 100% reasoning, 3 fallbacks (2 noop_row, 1 http_error), 0 truncated, off_protocol 68 (3.6%), plan_line_missing 73 (3.8%), p50 90.1 s. ⚠ sibling_window_asks_skipped 0 and stop_reached_windows_skipped 0 (BU predicted ~19; BT built the stop collapse) — engine seat must explain (unreachable after BX's narrowing, no qualifying window, or unwired counter). askreplay/ harvested (29 files, 521 reserved, 0 refused). Wins: 126 5/6, 146 5/6, 162/152 3/6, 125/130 2/6, 123 1/6. Step 3 launched (7 deck seats + engine seat), brief 7eadd5926.

## 2026-09-07 23:30 — WAVE-72 STEP 3 DONE; WAVE-73 CUT (base 90f8d1d68). Invariant 000 held (1,910/1,910 reasoning, 0 trunc, 0 hung, 3 fallbacks all genuine). Zeros explained: sibling collapse dead because the answered-tag is inside the board key (N1); stop collapse dead because the stop grammar is never taught/parsed (N3). THE HOLD FAMILY = N2 (Teferi silenced for a game, Create row held 15 turns, own-turn hold skipped 4 turns, pass row missing on 36/66 priority windows). Rung menus sort lexicographically (N4). Wave-71 guide edits PAID broadly; wave-72 EDITs installed 34abf1a66 (deck123's wrapped PLAN template joined to one line — the guard flags a second template line). Wave 73 lanes: BY (N1 N2 N5 N6 N9 N16), BZ (N3 N4 N7 N10 N11 N15), CA (N8 N12 N13 N14 N17). {X}-announcement loss: 0 fires this wave — docket kept, not built on.

## 2026-09-07 22:20 — WAVE-73 STEP ONE MERGED (base 90f8d1d68): BZ 0c98ff615 -> 4ea14e357 (clean; N3 repeat bracket states `stop=N; M=N`, PLAN-line-only reader accepts stop=/stop N/stop count N/stop at N + M=/M N/M is N/M at N (pins incl. their-stop + CHOICE-line MUST-NOT-MATCH); N4 naturalTextLess numeric sort; N7 revealCostTag; N10 exampleCoreTrim balanced; N11 board-frame loyalty clause; N15 noop re-ask quotes the match). CA 0d7a0d44d -> aa76e62c8 ((g) clean; N8 replay_run per window + repeat_ask path bounded; N12a menuHasReserveRow was DEAD since BV's reword (matched wave-67 words) — re-armed; N12b MEASURED and REFUTED: plain declines 105/233 are informed choices (101 weigh the hold row, 95 name a later action), cost 4.1% of calls — no change; N13 REFUTED (check exists, reaches land_drop 31/325 — the case authored its own false premise); N14 four fixed; N17 heading loses the caps phrase). BY b09de3f5f -> d2086e41b ((ci) stray brace; N2 hold EXPIRES at the holder's untap (`hold_released_turn`), pass row on EVERY priority window, cost clause leads all three hold spellings; N1 sibling key normalises the answered-tag (F1 subset bar still binding); N5 one loop-scope sentence shared; N6 chainAutoPassApplies generalised (oracle says no legal action) `chain_windows_collapsed`; N16 instrumented `main_phase_windows_skipped`, no RED). PARSETEST 5574/0. Gate 1 (THREADS=1) + Astra review launched (diff 90f8d1d68..d2086e41b).

## 2026-09-07 23:20 — WAVE-73 REVIEW + CB. Gate 1 THREADS=1 on d2086e41b 1281/0, 76 AI/0, PARSETEST 5574/0. Astra review (wave73/codex-review.md, 7 findings, all CONFIRMED by CB 9c0582928 -> 622b10827 clean): F1 chain collapse now judges legality with the seat's own GptManaPolicy (Lotus Petal) + phase-restriction filter mirrors isReactingToClick (upkeep-only abilities visible — also fixes the human ability border) — fixtures RED on base; F2 hold expiry stated as a pure predicate + eager release at WEventPhaseChange/every tick; F3 stop binds only as a COUNT (deny list life/mana/cards/turns/damage/points/poison/loyalty/lands) + clause-wide ownership; F4 per-row life fact in the loop note; F5 ghostform gloss only while the trigger is live; F6 main-phase meter once per phase at phase end; F7 replays_this_game one total. PARSETEST 5593/0; suite 1283/0, 76 AI/0 (2 new fixtures). Gate 2 (make -B, THREADS=1) running on 622b10827.

## 2026-09-07 23:00 — WAVE-73 STEP ONE CLOSED: gate 2 on 622b10827 (make -B, THREADS=1) 1283/0, 76 AI/0, PARSETEST 5593/0. Archive archives/wagic-4b90ed7d7-w73step1 (source 622b10827); pushed; worktrees removed. CORPUS w73 launched: unit w73-corpus, --thinking on, script ~/.gatelogs/w73-corpus-launch.sh, done-file W73_CORPUS_DONE. Predictions to adjudicate: hold_released_turn>0 and no hold outliving the holder's untap (BY/CB F2), pass row on every priority window + hold-as-pass takes ≈0 (BY N2d), sibling_window_asks_skipped>0 only with both option sets recorded (BY N1), stop parsed (the repeat-row stop clause renders >0; BZ N3) and stop_reached collapses only on the seat's own count-stop (CB F3), chain_windows_collapsed>0 with no legal action lost (BY N6/CB F1), rung menus numeric (BZ N4), reveal rows carry cost (BZ N7), reserve_decline_windows_noted>0 (CA N12a), 0 hung, decisions/turn ≈2.1.

## 2026-09-08 16:30 — WAVE-73 CORPUS: PILOT vLLM ENGINE WEDGED at ~17h; 15/21 natural, 6 games RERUN. matchups-20260907-225637 gate PASSED (210/210 reasoning); 15 games finished naturally (0 hung, 0 crashed) BEFORE the pilot (spark vllm-qwen36-35b, MTP=4 + fp8 KV + triton_attn) FROZE mid-generation: last engine log line 07:54:34 container-clock "Running: 7 reqs, generation 0.0 tok/s", no engine line after, GPU pinned 96% with zero throughput, API front end still answering /v1/models 200 (so nothing tripped). The 6 games still live (123v130, 125v123, 125v126, 125v130, 146v125, 152v125 — every deck125 game among them, by chance of latency) then ran ~8h on 900-s timeout fallbacks (257 timeout records, ~50 per game; latency climbed 55 s -> 194 s -> timeout before the freeze) — heuristic games, NOT corpus games (invariant 00). Found on the owner's "check on it" (the 4-h late check at 15/21 + 22 active logs looked normal). Actions: stopped w73-corpus (harness exit trap harvested 42 seat logs / 3,321 decisions and wrote results for the 15 games); `docker restart vllm-qwen36-35b` (ready in ~90 s; probe completion OK, 937 reasoning chars in 3.9 s); RERUN of exactly the 6 lost matchups as unit w73-rerun (script ~/.gatelogs/w73-rerun-launch.sh: four concurrent harness pools 125,123,130 / 125,126 / 125,146 / 125,152 -> outdirs matchups-20260907-225637-rerun-{a,b,c,d}; DEDUPE seat logs by basename when aggregating — concurrent harvests can copy a later game's log into more than one outdir; done-file W73_RERUN_DONE). Watches: gate+survival, terminal, 4-h late, plus a one-shot WEDGE TRIPWIRE (3+ timeout-fallback records -> wake). Forensics in ~/.gatelogs/w73-vllm-wedge-forensics.txt. LESSON (harness): a wedged pilot is invisible to the hang watchdog — the seat keeps ticking on heuristic fallbacks and translog mtimes advance; the late check should report timeout-fallback counts, not only active logs. DOCKET (not built on): phase-2 forced-close request rejected by the pilot with HTTP 400 `continue_final_message is set but the final message does not appear in the chat after applying the chat template` (qwen3.6-enhanced.jinja) — 1 of 6 forced closes in this corpus fell to http_error; 3 such errors in the server log; mechanism UNTESTED (template strips the reasoning prefill in some shape). STEP 3 for wave 73 waits on the rerun; the 15 finished games' seat logs are valid (all decisions pre-freeze carry reasoning; the 6 unfinished games' partial logs are in the corpus dir too — EXCLUDE the 12 seat logs of the rerun matchups from stats, or restrict to pre-timeout records).

## 2026-09-08 17:05 — OWNER RULING (watch cadence, verbatim): "if the late timer goes off, and no problem is found, a new timer, slightly in excess of the expected remaining runtime should be set." Recorded as Refinement 2 in fleet memory feedback_single_wake_batching (the procedural home of the cadence; the skill only says "arm a Monitor on the done-file"). Applied to the w73 rerun's late check from its next firing.
Clarified (owner, verbatim): ""no problem found" means that you check for the reason that its taking longer than expected, and finding that answer not to be a problem, you set a new timer according to the new expectation." The late check is a diagnosis (name the reason for the overrun), and the new timer is sized from the expectation that reason yields.
And (owner, verbatim): "alternatively if a problem is found, you would solve for that problem before starting a new timer set as usual." Problem branch = fix first, then arm the next timer from the post-fix expectation.

## 2026-09-08 20:35 — W73 RERUN late check 1 (4h): 4/6 done, 0 timeouts; open = 125v123 (t51) and 125v126 (t53). Diagnosis: deck125 (Elixir/wrath/counter control, one Emrakul as its only kill) plays 50-70-turn games by nature (this matchup ended t32 wave 72, t48 wave 71; a 125 game reached t70) — not a problem; next check armed at +2h. ALSO: corpus game 126v123 (seat logs 1788839841-*) FINISHED but its last 2+5 decisions were timeout fallbacks under the wedge -> not a full GPT game -> rerun E launched (unit w73-rerun-e, outdir ...-rerun-e, done-file W73_RERUN_E_DONE). So the wave-73 corpus = 14 clean corpus games + 7 rerun games. Stats script ~/.gatelogs/w73-stats.py (dedupes by basename, excludes seat logs with any timeout record or no gameend) — run after ALL harvests; then build a consolidated dir for the seats.
Late check 2 (+2h, 22:18): rerun E done (126 beat 123, 108 decisions). One game open, 125v123 at t67: reason = Staff of Nin clock (deck123 16 -> 10 over ten turns, deck125 at 100 life behind wraths); converging to ~t87 at 8.6 min/turn -> next check at +3.5h.

## 2026-09-08 23:05 — WAVE-73 CORPUS COMPLETE (21 clean games) + STEP 3 LAUNCHED. Rerun a-d + E all natural (125v123 ended t76 on the Staff clock before late check 3 — that timer and the wedge tripwire were stopped). Consolidated dir `~/.Wagic/ai/gpt/selfplay-runs/matchups-20260907-225637-final` = 14 original + 7 rerun games (42 seat logs, 14 timeout-tainted logs excluded, merged results.tsv with the 7 original rows of the rerun matchups dropped, askreplay 25 files, stderr files copied). Stats (~/.gatelogs/w73-stats.py): 1,757 model decisions, 100% reasoning (median 5,709 chars), 0 fallbacks, 0 truncations, 0 action_before_plan, 49 off-protocol (2.8%), 51 plan_line_missing (2.9%), latency p50 60.8 s / p95 169.0 s (mixed load), decisions/turn 1.89; gameend sums: hold_released_turn 101, stop_reached_windows_skipped 366 (w72: 0), sibling 0, chain 0, main_phase_windows_skipped 375, reserve noted 26 / skipped 2, replays reserved 1,262 / refused 0, async_drops 43, phase2_recovered 2. Wins 152 5/6, 162 4/6, 146 4/6, 130 3/6, 123 2/6, 126 2/6, 125 1/6. Brief filled + committed cec9d5025. Seats launched (Opus): 146/152/125/126/162/123/130 + engine seat.

## 2026-09-09 00:20 — WAVE-73 STEP 3 DONE; WAVE-74 CUT (base c0551632e). All seven seats EDIT, guides installed 0f916bd69 (all ≤19,995 B, A332 0, guard OK). Engine seat: census corrected to 2,338 decisions / 2.52 per turn (ask-seam-only figure was 1,757); hold expiry PASS on 101/101 releases; pass row 416/416; stop collapses 366 all deck123's own creature counts; sibling/chain zeros are safe-direction (subset bar / no empty acting row). HEADLINES of wave73/known-bugs.md §O (27 items): O1 ghostform gloss FALSE 56/56 (CB F5 inverted — Kaya grants the trigger on the creature, the scan reads the action layer); O2 declare-attackers window silently skipped upstream of pendingCombatDecision (3 turns, 1 lost game); O3 Silverquill +3/+3 target picker under the FORCED SACRIFICE header; O4 land-drop menu hides an MDFC face (drop decided twice); O5 ATTACK TOTAL verdict ignores the seat's own Sanguine Bond; O6 hold key ignores the projected-life carve-out (11 windows/turn re-opened); O7 plan carry strips the `stop=;M=` clause (planStepEnds opens a step at `;`); O8 X cast row unpriced; O9 no row prices the seat's own Staff clock (Emrakul at 131 vs 2, dead). Lanes: CC render truth (O1 O3 O5 O12 O15 O16 O19 O20 O21), CD window model (O2 O4 O6 O10 O13-measure-only O23 O26-harness-stall-tripwire), CE stop/plan + pricing (O7 O7b O7c O8 O9 O11 O14 O17 O18 O25); CF = Astra fix lane after gate 1. Worktrees worktrees/lanes/w74-{CC,CD,CE} seeded from c0551632e; done-files ~/.gatelogs/W74_{CC,CD,CE}_DONE.

## 2026-09-09 02:20 — WAVE-74 STEP ONE MERGED (base c0551632e): CD d57dd9756 -> d00d56fb0 (clean; O2 ROOT CAUSE ≠ the review's hypothesis: pendingCombatDecision PASSED — the forced-close / retry / ceiling-re-ask arms consumed the reply, armed the follow-up and returned kChoicePending with the async slot EMPTY, so decisionPending was false and Act passed priority over a live decision; plus the per-seat retry slot let the land-drop and casting seams destroy each other's armed leg — stub-endpoint RED/GREEN 409 armed/38 delivered -> 7276/7092; O4 both MDFC faces on the land-drop menu; O6 hold key normalises projected life; O10 meter rewired; O13 counter only; O23 trace; O26 harness PILOT STALL tripwire validated on the real wedged run (STALL 36) vs clean (OK 0)). CC f76a379a3+0cb1b2873 -> 1693c7f61 ((ci) stray brace; O1 root cause: `all(...)` makes the granted trigger an ALord with no getMenuText, structural branch now reads the grant; O3 notaTarget discriminator with a fixture; O5 converter arithmetic; O12; O15; O16 `(keeping 7)`; O19 X menus ASCENDING — reverses wave-23's largest-first order, riskiest change, no live ANNOUNCE_X yet; O20; O21 hold-text shrink refused by the lane). CE 1d79f0654 -> 92843101e ((b)+(ci): stray brace + the mRetryArmLand initializer spliced into a comment — restored by byte-splice; O7 planStatePrefixEnd makes stop=/M= state; O7b/O7c; O8 keep-X; O9 own-clock tag (no lethal-incoming gate); O11 decline row TAGGED; O14 fold; O17 placeholder-free name + deck123 guide; O18 range collapse, option set intact; O25 instrument). PARSETEST 5708/0, guard OK. Gate 1 (make -B, THREADS=1) unit w74-gate1 + Astra review (~/.gatelogs/w74-review-prompt.md, diff c0551632e..92843101e) running.

## 2026-09-09 03:10 — WAVE-74 REVIEW + CF. Gate 1 THREADS=1 on 92843101e 1285/0, 76 AI/0, PARSETEST 5708/0. Astra review (wave74/codex-review.md, 9 findings, all CONFIRMED by CF a9ca4b3e9 -> cfcc37a8f clean): F1 per-arm GptRetrySlot + park (the shared retry record was still overwritten across arms); F2 ownership stamped once in selectRetryArm; F3 harness stall predicate = newest K across seats by mtime + every active seat has evidence (Astra's three cases reproduced on base, now OK/STALL/STALL); F4 PILOT_SILENT_CLASSES shared by tripwire + banner; F5 lifeToDamageConverterRate (Cliffhaven 1-per-event, double Bond ×2); F6 keep-X from total payable mana after the fixed cost; F7 clock needs a recurring source + lethal-incoming gate; F8 battlefield-granted haste; F9 thousands separator inside the state clause; O19 render→parser→apply round-trip case. Counterfactual RED 5732/16. PARSETEST 5748/0; suite 1285/0, 76 AI/0; harness selftests 4/0, 13/0, 30/0. Gate 2 (make -B, THREADS=1) running on cfcc37a8f; scripts staged: ~/.gatelogs/w74-gate2-launch.sh, ~/.gatelogs/w74-corpus-launch.sh (archive hash to fill).

## 2026-09-09 03:20 — WAVE-74 STEP ONE CLOSED: gate 2 on cfcc37a8f (make -B, THREADS=1) 1285/0, 76 AI/0, PARSETEST 5748/0. Archive archives/wagic-4433a1199-w74step1 (source cfcc37a8f); pushed; worktrees removed. CORPUS w74 launched: unit w74-corpus, --thinking on, -j 21, script ~/.gatelogs/w74-corpus-launch.sh, done-file W74_CORPUS_DONE; the harness now carries its own PILOT STALL tripwire (CF F3), so no separate wedge monitor. Watches: gate+survival, terminal, late check at 8h (predicted maximum; chain per the 2026-09-08 cadence ruling). Predictions to adjudicate: O2 — no own turn with `able to attack` > 0 and no attackers record and no combattrace line (the async-arm repair; new entry trace names every such turn); O1 — ghostform gloss full on every live grant, marker 0 unless abilities removed; O3 — no FORCED SACRIFICE header on a buff-mode target picker; O4 — land-drop menus list every MDFC face, no double-decided drops; O5 — ATTACK TOTAL verdict never contradicts the log outcome under a converter; O6 — `rows above are new` never fires on a life-number-only change; O7 — every carried PLAN keeps its stop=/M= clause (deck123); O10 — main_phase_windows_skipped ≈ the honest 142-class figure, 0 overlap with casting decisions; O11 — declined-count decline takes fall from 40%; O13/O25 counters report (no behaviour); O16 — `(keeping 7)` on every first mulligan window; O19 — ANNOUNCE_X index = X, 0 echo_index_conflict; O26 — 0 PILOT STALL banners on a healthy pilot.
Early check (03:05): gate still WAIT after 1h (a seat with 3 of 5 records) — pace is SLOW, not stalled: turns 1-11 at 64 min, p50 latency 105 s, max 472 s under -j 21, deck123 seats at 35-37 asks by turn 2 (many windows per turn — engine seat should count them), seat logs all fresh. Expect a longer corpus than wave 73. NOTE for the harness docket (wave 75): (a) testing pilot_stall_verdict against the shared live logs dir is CONFOUNDED — a concurrent run's fresh logs are the newest by mtime and fill the pool (my "OK 0" was that, not a predicate failure; CF's isolated-dir validation stands); (b) the K pool takes "newest log first, its newest records first" so it can fill from ONE seat — a lone seat's 6 silent fallbacks would STALL a corpus whose other 20 seats answer; the banner says "every live seat" — make the pool take at most ceil(K/live) per seat, or require ≥2 seats in the pool. My own 3-fallback wedge tripwire monitor is armed as insurance for this run.

## 2026-09-09 10:15 — WAVE-74 CORPUS STOPPED AT 8h — INVALID (invariant 00): matchups-20260909-015553 (renamed …-INVALID-windowloop) 10/21 finished, 11 frozen at turns 3-15. ROOT CAUSE (evidence, not hypothesis): the O11 tag (CE, `declineRowReaskTag`) writes the incrementing declined-count INTO the decline row's text; the ask key is boardKey + the rendered option list, so no two windows match; `ask_replays_reserved` 0 / `identical_ask_answers_reserved` 0 on every seat (w73: 14/5); 12 seats declined one list 100-382 times in one turn (w73 ≤ 3), each a full model call — the same casting question re-asked every tick after `Cast nothing` with an unplayable land in hand. The regime gate never PASSED (a frozen seat stayed at 3 records) — my gate monitor timed out at 1h and I read the pace as "slow, not stalled" at the early check: WRONG — the tell was there (deck123 seats at 35 asks by turn 2). Lesson for the early check: count asks per (turn, phase) per seat; > 10 identical-phase asks on one seat = a window loop, stop and look. Second candidate (O2 per-arm pending) to be ruled in/out by the lane. Lane CG (brief 4ba920922, worktree w74-CG): move the clause out of the key, PARSETEST key-stability pin, suite fixture RED on base, decisionPending pin, harness WINDOW LOOP tripwire. Then gate → new archive → full corpus rerun. The 10 finished games are NOT step-3 material (their seats passed through the loop too).

## 2026-09-09 11:00 — CG MERGED 1d1d3744d (clean): the count leaves both ask keys and the async slot key (`stripRenderAnnotationsLc` protected only the option-set key); CD's per-arm park ruled OUT as load-bearing (bounded; the bound pinned firing) but an armed prompt carrying the rising count reset decisionArmed's time bound every window — pinned MUST-NOT-MATCH; tools/window-loop-probe.sh (stochastic: base looped 2/5 with counts 2028/2440, fixed 5/5 at 8-9 — but base runs that finished still reached 17, so "why is the window re-put ~8 times a turn" is open O10/O13 work); harness `== WINDOW LOOP` tripwire (LOOP 382 on the killed run, OK 0 on wave 73). PARSETEST 5760/0; suite 1285/0, 76 AI/0. Gate 3 (make -B, THREADS=1) unit w74-gate3 running. New tool ~/.gatelogs/corpus-early-check.py <launch-script> — asks per (turn,phase) per seat + declined-count max; the early check runs it.

## 2026-09-09 11:20 — GATE 3 on 1d1d3744d (make -B, THREADS=1) 1285/0, 76 AI/0, PARSETEST 5760/0. Archive archives/wagic-60e9fe2ad-w74step1b (source 1d1d3744d; the earlier wagic-4433a1199-w74step1 is INVALID for corpora); pushed. CORPUS w74 RELAUNCHED (unit w74-corpus, fresh done-file; launch script touched so corpus-early-check.py keys on this run). Watches: early check (gate verdict or +25 min, then the per-seat (turn,phase) ask census), terminal, late 8h chained. Harness tripwires now: PILOT STALL + WINDOW LOOP.

## 2026-09-09 18:40 — WAVE-74 CORPUS RELAUNCH (matchups-20260909-104713) STOPPED BY THE HARNESS at 18/21 — the new WINDOW LOOP tripwire fired on a FINITE chain (false positive), which exposed regression #2. Game 125v126 deck126 seat t32 Upkeep: 41 casting asks (all model calls, all `Hold priority`) while its OWN Sanguine Bond + Exquisite Blood chain drained the opponent 21 → 1 — one link from winning when killed. The hold was re-opened at every link: the decline row's O9 own-clock tag counts down with the opponent's life ("reaches 0 in 11 more turns"), so "a row changed". Second instance deck125 seat t27 Draw, 38 asks under the opponent's Ob Nixilis + Underworld Dreams chain. Wave 73's same matchup: ≤5 asks per phase. deck152 (17) / deck146 (15) are legitimate state changes. Early check (+25 min) was clean — this shape only appears late (chains). Lane CH (brief 194290438, worktree w74-CH): hold key = action identity with all render annotations stripped (O6's targeted normalisation was the wrong shape); pins; chain fixture RED on base; harness tripwire requires an UNCHANGED BOARD (a moving-life chain is not a loop). 18 finished games are VALID (the regression costs redundant asks, not legality); missing matchups 152v125, 125v126, 125v123 → rerun on the CH binary (mixed-binary corpus — the brief must say so; the engine seat's hold regression watch splits by binary). Corpus-early-check.py also confounded by CG's probe logs in the shared dir earlier (72 seats) — key it on the launch mtime, which the relaunch did.

## 2026-09-09 19:30 — CH MERGED aca5e06f8 (clean): the hold latch keyed on a RENDER key (holdKeyRow, six enumerated life anchors) — wrong key shape; instance 1 was O11's count then O9's clock tag per link, instance 2 (125v162 deck125 t27 Draw) was O8's X-pricing bracket restating the damage on the stack — a THIRD annotation class. Fix: holdActionKeyRow = row minus the combat-next clause, then stripRenderAnnotationsLc (hold latch only; O6's re-open bracket keeps the render key); J9's lethality fact moved to a marker row; sibling rule reads lower-cased keys. tools/hold-chain-probe.sh: base held 0 windows in 5/5, fixed 63-65. Harness WINDOW LOOP needs an unchanged board (K=8): chain run LOOP 40 → OK 0, dead loop still LOOP 382. O13 report: the chain-collapse instrument runs at the priority seam only — the 41 cast-seam windows were invisible; 416 of 2428 acting windows (17%) offered only `this does nothing` rows. PARSETEST 5788/0; suite 1285/0, 76 AI/0. Gate 4 (make -B, THREADS=1) unit w74-gate4 running; then archive w74step1c, push, rerun the 3 games (~/.gatelogs/w74-rerun-launch.sh).

## 2026-09-09 19:05 — GATE 4 on aca5e06f8 (make -B, THREADS=1) 1285/0, 76 AI/0, PARSETEST 5788/0. Archive archives/wagic-76c5fab24-w74step1c (source aca5e06f8); pushed. RERUN of 152v125, 125v126, 125v123 launched (unit w74-rerun, three concurrent pools -> matchups-20260909-104713-rerun-{a,b,c}, done-file W74_RERUN_DONE); watches: early check (+25 min census), terminal, late 8h chained. The wave-74 corpus = 18 games on wagic-60e9fe2ad (CG) + 3 on wagic-76c5fab24 (CH) — MIXED BINARY, the brief must say so.

## 2026-09-10 00:40 — WAVE-74 CORPUS COMPLETE (21 games, mixed binary) + STEP 3 LAUNCHED. Rerun a/b/c all natural (152 beat 125, 126 beat 125, 123 beat 125). Consolidated `matchups-20260909-104713-final` = 18 (CG binary 60e9fe2ad) + 3 (CH binary 76c5fab24); 42 seat logs, 10 partial copies excluded. Stats: 2,396 decisions (1,866 ask + 367 priority + 93 attackers + 40 discard + 17 blockers + 12 reveal + 1 bottom + 4 recovery), 100% reasoning (median 6,004 chars), 4 fallbacks (1 http_error, 2 unparsed_reply, 1 noop_row_zero_reask), 1 reply_truncated, off_protocol 4.6%, plan_line_missing 5.0%, latency p50 81.8 s / p95 183.1 s; gameend sums: ask_replays_reserved 584 / identical_ask_answers_reserved 1 (w73 1,262/999), hold_released_turn 70, stop_reached 30 (w73 366), sibling 0, chain 0, chain_windows_only_selfharm 6 / selfharm_rows 560 / acting_rows 1,123, main_phase_windows_skipped 72 (w73 375), plan_names_stranded_card 3, async_drops 84 (w73 43), phase2 recovered 10 / missing 2. Wins 152 5/6, 146 5/6, 162 4/6, 123 3/6, 126 2/6, 130 1/6, 125 1/6. Brief wave74/reviewer-brief.md (70701bb76). Seats launched (Opus): 146/152/125/126/162/123/130 + engine seat.

## 2026-09-10 02:10 — WAVE-74 STEP 3 DONE; WAVE-75 CUT (base 207a7f410). All seven seats EDIT, guides installed cce04d4af (19,943-19,996 B, A332 0, guard OK); synthesis ADOPTED deck162's two skill proposals as drafted Amendments 333 (a heading is a scope declaration) + 334 (no concessive clause for a rule's hard case) in wave74/strategy-writing-skill-v2-amendments.md — OWNER folds in (v2 untouched). Engine seat census corrected: 2,396 decisions / 2.62 per turn. PAID: O1 81/81, O2 33/34 (miss traced), O5 2/2, O7 20/20 (deck123 wave-73 HIGH-1 CLOSED), O16 42/42, O19 33/33 ascending, O23 discharged (53 mana lost on deck125 Revelations), CH hold held across an 84-link Bond chain with 0 asks. HEADLINES of wave74/known-bugs.md §P (24 items): P1 O11 tag net negative (takes 50.6%; its count killed run 1 and re-opened holds on CG) → REVERT the in-row tag + hold-check bracket on the action key; P2 phase-2 prefill HTTP 400 cost an ATTACK DECLARATION (2/12 forced closes) + 5/17 forced closes leave no record; P3 Exquisite Blood falsely `cannot pay now` (Battlement per-defender mana) — decided a game; P4 Sorin −6 charges 3, does nothing, second activation allowed; P5 loop-fed self-damage row prints a false `you would be at 41` (lost 42-7); P6 CRACK-BACK absent from all own upkeep/draw windows; P7 crack-back cover counts a legend-rule duplicate; P8 no casting re-ask after a countered spell. Lanes: CI window/hold (P1 P8 P13 P18 P19 P12 P24), CJ transport (P2 P21 P9 P14 P20 — the only lane allowed single probe requests to the pilot), CK engine truth (P3 P4 P7 P22), CL render truth (P5 P6 P10 P11 P15 P16 P17 P23); CM = Astra fix lane. Brief LESSON: every new bracket with a board number ships a key-stability pin across all five keys. Worktrees worktrees/lanes/w75-{CI,CJ,CK,CL} seeded from 207a7f410; done-files ~/.gatelogs/W75_<L>_DONE.

## 2026-09-10 ~03:30 — wave 75 step 1 merged (CI, CK, CJ, CL) = 301a30a37
Lane results: CI e5e611520 (P1a tag revert, P1b action-key hold-check, P13 offered-slot overwrite root cause, P8 REFUTED + trace, P18 MDFC faces, P19 hold paragraph rewritten true, P12 census, P24 early-check on manifest + STALL pool bound; PARSETEST 5809). CK 79510b68b (P3 planPayment skipped foreach-wrapped variable producers in every pass; P4 premise REFUTED — -6 charged 6, real bug Sorin self-target fixed in card data + new `assertcantarget` suite cmd; P7 legend-rule dup; P22 both parse messages false; suite 1288/0, PARSETEST 5795). CJ a80f9d0ff (P2 root cause = Qwen template |trim on the prefill -> continue_final_message 400; gptForceClosePrefillBody fixed-point pin; P21 = max_tokens bounds whole decode, arm extended to cap-with-no-answer; P9 inline CHOICE:; P14 re-ask restored; P20 8-gram meter; PARSETEST 5826). CL a464f4a67 (P5 feeds-their-chain, P6 crack-back every own window, P10 own-clock gate on worst stated loss, P11, P15 descending ladders, P16 reviewer wrong — real defect 60-byte mid-word cut, P17, P23d; PARSETEST 5835).
Merge seams (3 hand-resolved): ctor initializer list twice (CJ +mForceCloseUnrecorded/mForceCloseArmed; CL +mPlanNamesUncastableZoneCard/mProtocolDeviationReplies — first attempt appended AFTER the trailing // comment = the eaten-initializer trap, fixed by inserting before the comment); pass-row render site (CI removed declineRowReaskTag, CL added two ownClockTagFor args — kept both); PARSETEST #W75-CL P5 KEY PIN held set changed holdKeyRow -> holdActionKeyRow to match the live seam (mLastMenuRows inserts action keys). Merged PARSETEST 5901/0; instruction guard OK. Gate 1 unit w75-gate1 + Astra review launched concurrently (prompt ~/.gatelogs/w75-review-prompt.md names all three seams for attack).

## 2026-09-10 00:36 — wave 75 step 1 SHIPPED = master 14fea99f3 (pushed); corpus LAUNCHED
Gate 1 (301a30a37): 1288/0, 76 AI/0, PARSETEST 5901/0. Astra review (wave75/codex-review.md): 9 findings, ALL CONFIRMED by lane CM (18465bf7a; 7 RED in one PARSETEST run 5901/9 on the seeded tree, F1 RED via suite, F9 via --selftest). Fixes: F1 new target-DSL `other!<type>` (Sorin -6 = `other!creature *[creature;planeswalker]`; CK's re-entry-latch paragraph SUPERSEDED — a returned walker is a new object, CR 400.7), F2 cost rides the hold action key, F3 inline CHOICE: needs a digit head, F4 one coded-answer reader, F5 armed flag in GptRetrySlot, F6 Python str.strip() Unicode set, F7 infect prints a poison forecast (source fix), F8 legend walk counts tokens, F9 PILOT STALL needs every live seat's NEWEST record to be a failure (+600 s recency grace — chosen, not measured). Gate 2 (14fea99f3, make -B): 1289/0, 76 AI/0, PARSETEST 5954/0; --selftest 4/17/12/30 all 0. Archive archives/wagic-7a04cd6da-w75step1 (ldd clean). All five lane worktrees removed.
Corpus: unit w75-corpus, ~/.gatelogs/w75-corpus-launch.sh (thinking on, 7 decks -j 21 -T 0, pilot qwen36-35b-a3b :8084 checked idle+answering before launch), started 00:36 CDT. Watches: early (tripwire line or +25 min → tools/corpus-early-check.py <outdir>), terminal, late at 8h chained per the owner's ruling. Predictions to score at step 3: CM weakest evidence = `other!` honoured only on the factory's common exit path; F7 never seen live (no infect source in the pool); F9 grace unmeasured.

## 2026-09-10 08:40 — wave-75 corpus late check (8 h): NOT a problem, timer re-armed
Early check (01:01, +25 min): 42 seats, 263 asks, no fallbacks, max 4 asks in any phase, no window-loop signature. Late check (08:36): 20/21 games finished; 125v123 in flight at turn 55-56, progressing (≤3 asks per phase on both seats, pilot Running 1 req at normal throughput; the `77 windows held` stderr lines are hold-latch replays, not model calls). Reason for the overrun = one long deck125 game (deck123 seat 418 asks over 8 h ≈ 69 s/ask, ~8.6 min/turn). New expectation ≤ ~20 more turns ≈ 3 h → late check 2 armed at +3.5 h (chained per Refinement 2).

## 2026-09-10 11:50 — wave-75 corpus COMPLETE (21/21 natural, single binary); step 3 seats launched
Corpus dir renamed `~/.Wagic/ai/gpt/selfplay-runs/matchups-20260910-003556-final` (started 00:36, finished 11:49 CDT; the 125v123 game ran 82 turns). 0 timeouts/crashes/hung, 0 reruns, no tripwire fired. Stats (`~/.gatelogs/w75-stats.py` → w75-stats.out; ⚠ the w74 helper HARDCODES its corpus dir and ignores argv — my first run printed wave-74 numbers; the w75 copy fixes the dir and counts ALL kinds): 2,919 decisions (ask 2,118 / priority 603 / attackers 102 / discard 53 / blockers 27 / reveal 10 / bottom 6), 100% reasoning (median 5,678), 0 fallbacks of any kind, 0 reply_truncated, off_protocol>0 71 (3.4%), plan_line_missing 83 (3.9%), latency p50 80.4 / p95 185.6 s, 2.15 ask-decisions/turn. Gameend: ask_replays_reserved 1,801 / identical_ask_answers_reserved 1,332 (w74: 584/1 — engine seat to explain), hold_released_turn 115, hold_windows_skipped 1,601 (cast 1,075 / priority 526), forced_close_unrecorded 9 (CJ predicted 0 — engine seat (c)), phase2 recovered 15 / missing 0, async_drops 70, main_phase_windows_skipped 94, protocol_deviation_replies 107, plan_names_uncastable_zone_card 68, casting-seam chain census first time (482/692/1,445). Wins: 152 5/6, 146 5/6, 162 3/6, 125 3/6, 126 2/6, 130 2/6, 123 1/6. Brief wave75/reviewer-brief.md committed 6973c996a; 7 deck seats + engine seat launched 11:55 CDT.

## 2026-09-10 13:10 — wave 75 step 3 CLOSED = master 77b920612 (pushed); wave 76 lanes CN/CO/CP launched
Seats: 7 deck seats ALL EDIT (guides verified: guard OK 37, ≤20,000 B, 0 citations, no "YOUR PLAN", PLAN templates one line; installed 21d2203cf), 0 proposals → no synthesis. Engine seat: 0 fallbacks of any kind, identity recovered+missing+unrecorded == closes in 21/21 (the 9 unrecorded = superseded arms, no decision lost; CM F5's "non-zero = lost" clause WRONG → Q8 rewrites it SUPERSEDED). PAID: P5 (deck125 declined 22 feeds-their-chain rows, won 38-0 on the board that lost 42-7), P6, P3, P4, P7, P18, P19, P2, P8, P10, P11, P22. FAILED: P13 still 36% false (Q7); CJ's forced_close_unrecorded=0 (Q8); P19's 650 B ceiling (6 rows, LOW). New HIGHs (§Q): Q1 hold-check referent = last BUILT window not last ASKED (214/2,000 windows), Q2 80,805-B prompt (uncapped THEIRS enumeration × uncollapsed copy rows), Q3 loop/stated-lethal false verdicts (combat header NOT lethal beside LOOP COMPLETE; own-clock beside crack-back KILL), Q4 aggregate cover ignores menace + floor false under Lightmine Field, Q5 crack-back asymmetry (kill rows get no cover), Q6 sacrifice row half-price under their converter. Q16 report-only (attackers seam drift 15.7%; unlabelled_plan 83%; declined-count "Cast nothing" 54%, six waves one direction). Docket: BS F3/land latch RETIRED. Wave76 brief: CN (Q1,Q7,Q8,Q13,Q9), CO (Q3-Q6), CP (Q2,Q10-Q12,Q14,Q15), CQ = Astra fix lane. Lanes seeded from 77b920612, launched 13:05 CDT. OWNER items pending: lategame specimen wave75/lategame-specimen.txt (alert-only); A333/A334 fold-in (wave74/strategy-writing-skill-v2-amendments.md) still not told.

## 2026-09-10 ~15:00 — wave 76 step 1 merged (CN, CO, CP) = f3eff598d; gate 1 + Astra running
Lanes: CN 1d6eb9a0a (Q1a hold-check memory committed only on windows that earn a record — new `mWindowSeq`; Q1b the residual false-NEW was NESTING (`stripRenderAnnotationsLc` stops at the first `}`), holdActionKeyRow now strips balanced groups; Q7 REFUTED — the stderr printed observer->turn while records print +1, 0/94 false under the right mapping; Q8 arm refused while the other arm's close is parked + `force_close_arms_refused`; Q13 `crossphase_identical_reputs` + prompt-only bracket, 273 in the w75 corpus; Q9 exemptions + the if/else under-count; PARSETEST 6008). CO 775b2ac4a (Q3 `w76StatedLethalOnScreen` shared predicate; Q4a knapsack over minBlockersRequired, Q4b floor requalified via the `attackPunisher` class; Q5 `crackBackKillRowTag`; Q6 converter clause on the sacrifice row; PARSETEST 5996). CP 59d2e987a (Q2 = `joinVictimRoster` count+shapes 9,250 B → 220 B AND `copyTagOwnsRow` — the discard fold DID run, `splitRowHandle` precedence keyed copy rows on a battlefield handle from their own roster; Q10-Q12, Q14 fixed; Q15 Lightmine Field REPORTED not changed — the script's foreach is by design, docketed for a card-script lane; PARSETEST 6016). All three merges CLEAN. Merged PARSETEST 6112/0 (= 5954+54+42+62 exact). Guard OK, harness bash -n OK. Gate 1 unit w76-gate1 + Astra (~/.gatelogs/w76-review-prompt.md) launched; worktrees removed.

## 2026-09-10 14:15 — wave 76 step 1 SHIPPED = master b4784ed05 (pushed); corpus LAUNCHED
Astra (wave76/codex-review.md): 10 findings, ALL CONFIRMED by lane CQ (e1bd0929e + d52e33e53). Fixes: F2 refused force-close arm now DEFERS (kChoicePending, no decode bought), arms when the park clears or at 8 ticks — pinned no-heuristic + resolves; F1 same-window early return re-stages; F10 `stripRenderAnnotationsLc` strips BALANCED groups (two inherited first-`}` pins #W67-AX I7 / #W69-BF K3 re-derived from the rule — both genuinely RED first); F5/F6 new suite cmds `assertcrackremoved` / `assertcangainlife` (F6 exemplar: Erebos can't be placed by a fixture's inplay list — fixture uses Grima Wormtongue; whether Erebos itself registers the prohibition is UNSETTLED); F7 roster K-named + `and N more`; F3 phase stripped from the re-put key; F4 board key compared (may be too strict — possibly a dead positive clause, fail-closed); F9 echo classified by byte match; F8 exemption needs a self-targeting graveyard/exile ability. Gate 2 (b4784ed05, make -B): 1291/0, 76 AI/0, PARSETEST 6168/0. Archive archives/wagic-d372a1153-w76step1 (ldd clean). All worktrees removed. Pilot checked idle+answering.
Corpus: unit w76-corpus, ~/.gatelogs/w76-corpus-launch.sh, started 14:15 CDT. Watches: early (+25 min census), terminal, late 8 h chained. Predictions to score at step 3: CQ F4 positive clause may never print; F6 Erebos unverified; CN Q1 live commit path argued not observed; CO Q5 removal cover has no live render yet; CP group cap untested on a real board.

## 2026-09-10 15:56 — PILOT WEDGE #2 (true-positive PILOT STALL); wave-76 corpus reruns launched
Engine log froze 15:42 CDT (`Running: 19 reqs ... 0.0 tokens/s`, container up 46 h, no error lines); API still answered /v1/models, a completion probe hung 60 s — same silent wedge as 2026-09-08 (memory spark_qwen36_mtp_crash). The wave-76 F9 tripwire (every live seat's NEWEST record a timeout) fired at 15:56 = 14 min after the freeze (wave 73: 8 h on fallbacks). Harness exit trap harvested; results.tsv holds 2 finished games (146v152, 152v162); 19 stalled at turns 7-15 with heuristic fallbacks = NOT corpus games. Fix `ssh spark docker restart vllm-qwen36-35b` (models up 90 s, probe 0.3 s). Reruns: ~/.gatelogs/w76-rerun-launch.sh = 19 concurrent two-deck pools (unit w76-rerun, outdirs matchups-20260910-141526-rerun-NN-<pair>, logs ~/.gatelogs/w76-rerun-NN.log, done-file W76_RERUN_DONE), launched 16:0x CDT. Aggregation at step 3: the 2 clean games' seat logs live in the SHARED logs dir (start epochs 1789067756 / 1789067732) — pick by epoch±6 s, dedupe by basename, exclude timeout-tainted; stats script dirs = the 19 rerun dirs + the 2 clean seats.

## 2026-09-10 22:50 — wave-76 reruns COMPLETE (19/19 natural) → corpus 21/21; step 3 seats launched
All 19 pools finished 22:36 CDT (0 timeouts/crashes/hung). CONSOLIDATION GOTCHA: each one-game pool's harvest "harvested 37 player-game logs" — the harness harvest keys on mtime and copied OTHER pools' in-flight seat logs mid-game into its outdir, so a `cp -n` across pool dirs took stale snapshots (26 of 42 logs without gameend). Rebuilt `matchups-20260910-141526-final` by taking the LARGEST copy per basename across the shared logs dir + all pool dirs (26 seat logs + 11 askreplay files replaced) → 42/42 gameend, 0 tainted; the 2 first-run games picked by start epoch (1789067756/1789067732); merged results.tsv 21 rows. Docketed: harness harvest keyed on the outdir manifest (as corpus-early-check.py). Stats (~/.gatelogs/w76-stats.py → w76-stats.out): 2,188 decisions (ask 1,594), 100% reasoning, 0 heuristic fallbacks (1 `repeat_count_reask` = P14's first live re-ask), 1.73 ask/turn, p50 92.4 s; forced_close_unrecorded 5 with the F2 refuse/defer counters all 0 (engine seat (c)); crossphase_identical_reputs 111; plan_names_uncastable_zone_card 102. Wins 152 5/6, 130 4/6, 162 3/6, 126 3/6, 146 2/6, 123 2/6, 125 2/6. Brief wave76/reviewer-brief.md committed 3ea5676cc; 7 deck seats + engine seat launched 22:48 CDT.

## 2026-09-11 ~00:10 — wave 76 step 3 CLOSED = master 49c19f413 (pushed); wave 77 lanes CR/CS/CT launched
Seats: 7 deck seats ALL EDIT (installed 3858f4e67; guard OK, ≤20,000 B, 0 citations), 0 proposals. Engine seat: identities hold (closes 18 = 13+0+5), 37 drops re-asked, Q7 refutation verified independently (0/56 record-turn vs 30% observer-turn), max prompt 30,375 B. PAID: Q2, Q3b, Q4, Q7, Q9, Q11, Q12, Q14, F3/F8/F10, P14 first live firing (`x62`), Q1 on the priority seam (0/1,011 non-land pairs false). NEW HIGHs (§R): R1 won deterministic loop asked 32× in a row (~48 min); R2 hold-check three residuals (land shares the cast seam — all 22 false brackets; streak-1 claims; the "N new" count wrong on reorder/disappear); R3 the Q6 converter clause read as a COST statement (lost 162v126) + mirror/opponent-branch unpriced (Devour Flesh); R4 blockers at-best ignores metered blocking lifegain; R5 battlefield line prints NO creature ability text (3,892 entries, 0 glossed). R12 report-only: 56% of decode after the first CHOICE token, protocol tail re-quoted. Stats helper now prints a CORPUS-WIDE census line (72/91/90.2 s = engine seat's figures) — the brief must use it. Docket: {X}-announcement loss RETIRED. Wave77 brief: CR (R2,R1,R8,R9,R11), CS (R3,R4,R6,R7,R10), CT (R5, R11-row), CU = Astra fix lane. Lanes seeded from 49c19f413, launched ~00:05 CDT. OWNER pending: wave75 + wave76 lategame specimens; A333/A334 fold-in.

## 2026-09-11 ~01:40 — wave 77 step 1 merged (CR, CS, CT) = 90643013b; gate 1 + Astra running
Lanes: CR e7f0a09e3 (R2a CONFIRMED with a new mechanism — the casting builder wrote `mNextAskPromptNote` then took a no-ask return, so 11 land-drop records carried a hold-check bracket they never measured; land gets its own seam, note cleared at three no-ask exits; R2b first-window sentence; R2c REFUTED — a faithful port reproduces the engine's counts, the cited "reorders" were real `legal targets right now:` roster changes; R1 half-refuted — seqs 30→32 key identically so the latch WOULD have held, the model was never told; annotation + `own_loop_windows_asked` + 5-link fixture 0 asks; R8 104/109 boards genuinely moved, comparison not too strict; R9 cast-intent window; R11 same-arm defer + harvest keyed on the outdir manifest, --selftest harvest 8/0; PARSETEST 6222). CS f6df8bb29 (R3 event-naming clause on sacrifice rows + both edict branches + own converter + `edictFloorScan` range; R4 AT BEST folds the metered gain; R6 cover on targeted-removal / X sub-menu / stay-home; R7 legality-filtered counts; R10 plan echo on ANNOUNCE_X; PARSETEST 6234). CT a7a77683b (R5 creature `{effect:}` gloss from text=, 224/450 corpus entries glossed live, seq-7 prompt 7,260 → 7,538 B, worst-case bound 37,121 B; R11-row rosters through joinVictimRoster 735 → 81 B; PARSETEST 6194). Merges: CR/CS clean; CT = one stray 8-space brace at the CT/CS PARSETEST seam (deleted). Merged PARSETEST 6314/0 (= 6168+54+66+26). Guard OK, harness bash -n OK. Gate 1 unit w77-gate1 + Astra (~/.gatelogs/w77-review-prompt.md) launched; worktrees removed; gate-2/corpus/stats scripts staged.

## 2026-09-11 ~02:05 — wave 77 gate 1 green (1291/0, 76 AI/0, PARSETEST 6314/0); Astra 10 findings → lane CU launched
Astra (wave77/codex-review.md): F1 HIGH the R6 pin ENSHRINES ask-key drift (cover numbers survive `stripDeclineReaskTags` into keyTailStr — the CG shape; an "older sibling" cited as precedent); F2 per-X cover sums targets without `sweep`; F3 targeted + per-X cover skip `w76CrackVictimTrulyRemoved` (CQ F5 re-introduced); F4 `w77OwnLoopResolving` fires on ANY stack object (the opponent's removal on Bond → "not needed" on the counterspell row); F5 a timed-out asked window loses its history commit (wall-miss record consumes mWindowSeq — CR's named suspect); F6 R2c pins miss target-preview drift (P/T in the roster = "new"); F7 blocking gain metered under a lifegain prohibition; F8 targeted-cover annotation enumerates 100 victims; F9 CT's 37,121 B bound omits snippet markers; F10 harvest tolerance is 15 s, same-pair pools indistinguishable. Lane CU seeded from 3752b3a68, launched ~02:05 CDT.

## 2026-09-11 01:02 — wave 77 step 1 SHIPPED = master 8e394a077 (pushed); corpus LAUNCHED
Lane CU 2dcbe3a9e: ALL 10 CONFIRMED. F1 ask + async-slot keys take the balanced-group strip (`w77KeyTailOf`), four pins re-derived incl. the `crackBackKillRowTag` "older sibling" and CT's roster pin; F5 wall_miss rides the in-flight window's seq (no longer consumes mWindowSeq); F4 three-way own-loop verdict IDLE/RESOLVING/THREATENED off the live stack + an `[own loop verdict:]` MARKER ROW so the hold re-opens (the `[...]` clauses alone would be stripped) — the threat clause moves the ask key by design (flagged judgement); F3/F2 one guarded contribution + one classifier shared by both cover walks; F7 one prohibition predicate gates every metered gain; F6 preview P/T out of the hold key (R2c refutation stands — separate defect); F8 6,212 → 404 B on 100 bodies; F9 snippet ceiling enforced, bound 38,657 B; F10 exact seat-log identity via `WAGIC_GPT_TRANSLOG_FILE` announcement, --selftest harvest 14/0. Gate 2 (8e394a077, make -B): 1291/0, 76 AI/0, PARSETEST 6402/0. Archive archives/wagic-42c41fc66-w77step1 (ldd clean). Pilot checked (up 9 h, probe 0.27 s). Corpus unit w77-corpus started 01:02 CDT; watches early/terminal/late armed. Predictions to score at step 3: CU F4's threat clause and marker row never seen live; F5 pinned at the function only; CR's own-loop annotation first live exposure; CT gloss on every board; CS R3 clause on a live sacrifice ask (0 last corpus).

## 2026-09-11 03:16 — PILOT WEDGE #3 (true-positive PILOT STALL); wave-77 reruns launched
Engine froze 03:01 CDT (`Running: 20 reqs ... 0.0 tokens/s`, GPU 96%, container up 11 h — restarted at 16:00 the day before, so ~11 h uptime and 2 h into the corpus). Tripwire fired 03:16 (15 min). 1/21 finished (126v162, epoch 1789106557). `docker restart` → models 90 s, probe 0.54 s. Reruns: ~/.gatelogs/w77-rerun-launch.sh = 20 two-deck pools (unit w77-rerun, dirs matchups-20260911-010157-rerun-NN-<pair>), launched 03:2x CDT. THREE wedges in four days (09-08 ~9 h in; 09-10 1.5 h in; 09-11 2.2 h in) under MTP=4 + fp8 KV + triton_attn at 19-21 concurrent ~20 KB-reasoning requests — the config lever (num_speculative_tokens=1, upstream-confirmed stable) is now the proposal to the owner; not changed unilaterally (costs speed; his serving config).

## 2026-09-11 10:45 — wave-77 reruns COMPLETE (20/20 natural) → corpus 21/21; step 3 seats launched
All 20 pools finished 10:31 CDT. CU F10's manifest harvest VERIFIED: every pool dir holds exactly its own 2 seat logs ("harvested 2 player-game logs"), consolidation into `matchups-20260911-010157-final` replaced 0 files, 42/42 gameend, 0 tainted, 21 results rows (first-run 126v162 epoch 1789106557 picked from the shared dir). Stats (~/.gatelogs/w77-stats.py → w77-stats.out): 2,740 decisions (ask 1,999; new kinds `recovery` 5, `defer` 1), 100% reasoning, ⚠ 6 HEURISTIC FALLBACKS (5 unparsed_reply + 1 noop_row_zero_reask — first since wave 74) + 1 phase2_answer_missing; corpus-wide off_protocol 120 (4.4%) / plan_line_missing 141 (5.1%) / p50 82.5 s; forced_close_unrecorded 10 with every arm-bound counter 0; own_loop_windows_asked 1; crossphase_board_unchanged 2 of 119. Wins 152 6/6, 146 5/6, 162 3/6, 125 3/6, 123 2/6, 126 1/6, 130 1/6. Brief wave77/reviewer-brief.md committed 78af55e95; 8 seats launched 10:45 CDT.

## 2026-09-11 ~12:30 — wave 77 step 3 CLOSED = master 1e465fff7 (pushed); wave 78 lanes CV/CW/CX launched
Seats: 6 EDIT + deck152 KEEP (6/6) — six guides installed 031db297b; 0 proposals. Engine seat: hold-check 2,174/2,174 referents correct, 0 false claims (R2 CLOSED); forced-close identity 15+1+10=26; F10 harvest verified two ways; R1 storm fixed (max 4 asks/phase); the own-loop RESOLVING verdict TRUE but the `[own loop verdict:]` marker never renders (key-only) — S6. THE FIVE unparsed_reply = one shape: PLAN line + `<n> (<short name>)` with NO `CHOICE:` label; two cost a held Path to Exile (the heuristic cast it) — S1 is OWNER-RULING territory (no parser reads outside the two labelled parts): routed MEASURE-ONLY (class `answer_label_absent`, `recovered_seq`, heuristic-played count), no parser change until he rules. Other HIGHs: S2 repeat-row instruction contradicts the closing sentence (339 s loop), S3 stop-reached row re-put 12× (collapse under the ruling's allowance), S4 9 identical windows during a 7-trigger drain (annotation + hold), S5 counter rows on a spell the seat's own counter already targets (lost a game), S6 marker row unrendered. S7 `hold_check_ref_seq` is a window ordinal — four seats + engine seat could not join it. Census: kind-sum 2,746 incl. recovery/defer. Wave78 brief: CV (S3,S4,S6,S7,S9,S11), CW (S2,S5,S8,S12,S13,S14,S15), CX (S1 measure, S10, tools/corpus-stats.py), CY = Astra fix lane. Seeded from 1e465fff7, launched ~12:30 CDT. OWNER pending: S1 ruling (may the parser read an unlabelled `<n> (<short name>)` answer line after a PLAN line?); pilot MTP=1 / max-num-seqs proposal; wave75/76/77 lategame specimens; A333/A334 fold-in.

## 2026-09-11 ~11:55 — wave 78 step one merged (CV, CW, CX), gate 1 + Astra review running
- Merges: CV `c688c288d` (clean), CW `a0078c467` (one ctor-init hunk auto-resolved "(ci)"), CX `818f4d338` (two hand seams: ctor-init UNION with `mAnswerLabelAbsentHeuristicPlayed(0)` inserted before the trailing `//#W74-CD` comment; PARSETEST tail — CV/CW block and CX block both appended at the same tail, unioned, then TWO closing braces re-inserted before the `#W78-CX` comment because the common tail only closed one block. Also one stray `=======` line survived the first splice because the CV section header comment contains a run of `=`; deleted by line).
- PARSETEST 6547/0 = base 6402 + CV 48 + CW 71 + CX 26 (exact). U+FFFD 0, reply-instruction guard OK (1 source, 37 guides), harness bash -n OK.
- Gate 1 unit `w78-gate1` launched (done-file W78_GATE1_DONE). Astra review launched (prompt `~/.gatelogs/w78-review-prompt.md`, out `w78-review-codex.out`).
- Lane weakest-evidence notes (from the lanes' own reports): CV S3 decline count inferred from the prompt; S4 7-link stack never live; S6 THREATENED face never seen in a corpus; S9 delta pinned on hand-transcribed keys. CW: first suite run exited after 1,304/1,367 tests with no summary, not reproduced, clean rerun reported; S13 does NOT touch the deck130 30,125 B max (that prompt is 21,918 B narration + three different row texts — the docket's attribution was wrong). CX: cross-seat defer routing never observed to move a record live; the live S10 green rode a forced buildMenuChoice failure; root cause docketed not fixed — buildMenuChoice takes the X ceiling from the ACTING seat's pool so a foreign seat's menu is always unaskable (decision-seam fix, next wave).

## 2026-09-11 12:55 — wave 78 step one CLOSED; corpus launched
- Astra review wave78/codex-review.md: 12 findings; Lane CY (worktree from dd88b21b8) CONFIRMED all 12 (report wave78/lane-CY.md): F1 collapse identity = hold-latch action key + ask-cache board key + stop statement; F8 hoisted card-text header stripped from ask + async keys (the CG shape, third time); F4 untargeted type-sweeper = THREATENED, any opposing object above the trigger = new UNPROVEN face; F3 conditional counters (pay/?/unless) priced from the script; F2 drain clause drops the legality promise, ACTION_ABILITY only; F5 gone-guard + one shared ordinal-stripped key; F11 continuation checks this arm + park; F6 quotes/parens atomic in the cutter; F7 GAINED/LOST from WEventLife, net-only without a source; F9 colour-aware selectAutoTapProducers forecast; F10 counter on final class (parser byte-identical, pinned); F12 corpus-stats generic kind list + --selftest.
- Merge CY `7c0f15835` clean. Gate 2 (make -B): 1291/0, 76 AI/0, PARSETEST 6642/0. Archive `archives/wagic-47ef378c8-w78step1` (ldd clean). Pushed. Worktrees w78-{CV,CW,CX,CY} removed.
- Pilot: idle before launch (Running 0), probe answered "OK" within the 60 s window.
- Corpus: unit `w78-corpus` 12:54 CDT, outdir `~/.Wagic/ai/gpt/selfplay-runs/matchups-20260911-125420`, 21 concurrent, thinking ON. Watches: early (+25 min, corpus-early-check.py), terminal (persistent), late 8 h (chained).
- CY weakest evidence to carry as UNTESTED: F1's board key may keep the 12→1 collapse from happening on live boards (numbers may move between re-puts — check `stop_reached_reputs_collapsed` in the census); F9 has no executed board; F4's UNPROVEN face may silence true verdicts and one old guard line was dropped; F7 depends on WEventLife reaching the seat for every opponent life change; F11 pinned one frame short of pollCompletionRetry.

## 2026-09-11 20:58 — wave 78 corpus late check (8 h): overrun diagnosed, not a problem
- 19/21 results at 8 h 02 min. Live: `123v125` (turn 61/62) and `125v126` (turn 81), seat logs 0-6 min old; pilot Running 2 reqs at ~85 tok/s, KV 2.4% — no stall signature. Overrun = two long grinding games (deck125v130 also went to turn 69 before ending). Re-armed the late check +2.5 h per Refinement 2.
- Fallbacks so far: 6 `unparsed_reply` all classified `answer_label_absent` (CX S1 measure), ALL on deck125 seats (4 in 123v125, 1 in 125v126, 1 in 125v130); 1 `engine_answered` (deck152 v 123 seq 22). The S1 shape is a deck125 habit; the owner ruling is still pending — the measure now shows it recurring wave over wave (5 in w77, ≥6 in w78).

## 2026-09-11 21:40 — wave 78 corpus COMPLETE; step 3 seats launched
- Corpus `matchups-20260911-125420-final`: 21/21 natural, single run, no pilot incident (first wave since 75 with no wedge); 42 logs all gameend, askreplay 30. Total 8 h 30 min (last two games turns 62/81).
- Census (tools/corpus-stats.py, ~/.gatelogs/w78-stats.out): protocol_replies 2,629; 8 heuristic fallbacks = 7 unparsed_reply (6 answer_label_absent, ALL deck125: 123v125 seqs 83/140/215/305, 125v126 seq 107, 125v130 seq 303; 1 unlabelled_plan deck162 seq 16 — no PLAN label and no CHOICE label) + 1 reveal/engine_answered (152v123 deck152 seq 22, latency −1). CENSUS CHECK off by exactly that one record (engine_answered reveal is a ROUNDTRIP kind that made no round trip) → tool item wave 79. plan_line_missing 103 (3.9%, was 5.1%), off_protocol 96 (3.7%, was 4.4%), p50 78 s. stop_reached_windows_skipped 44 / stop_reached_reputs_collapsed 0 (S3 collapse never fired — CY F1's board-key risk, regression watch (b)); stack_drain_windows_asked 60; own_loop_windows_asked 2; forced_close_events 32 / unrecorded 16 (was 10); phase2 recovered 0 / missing 0 (was 15/1); mana_only_windows_skipped 367 (was 69); hold_windows_skipped 2,816 (was 2,128). Wins 146/126/152/123 4/6, 125 3/6, 162 2/6, 130 0/6.
- Brief wave78/reviewer-brief.md committed b0c04a926. Eight Opus seats launched 21:40 (7 deck + engine; engine quotes all 8 fallbacks first — the S1 ruling evidence).

## 2026-09-11 22:35 — wave 78 CLOSED (step 3); wave 79 step one launched
- Eight seats done: all seven decks EDIT (guides installed a37b2d5ce; all ≤20,000 B, cites 0, guard OK), no proposals (no synthesis), engine seat + specimen 90b29b159, known-bugs §T + wave79 brief 06fad1449 (pushed).
- §T HIGHs: T1 S1 ruling still pending (6 label-absent, all deck125; real cost once — a 12-mana Revelation stranded; deck125's guide lacked the literal `CHOICE:` — edit 4 pairs it; watch the counter); T2 S3 recurs, ROOT CAUSE: `w72StopReachedWindowCollapses` needs planTurn == nowTurn and CY F1 needs a repeat row (40/44 windows had none) — 90.8 min decode for 3 non-pass answers; T3 the CG shape a FOURTH wave — 419/1,454 same-seam re-asks byte- or strip-identical (578.8 min decode), hold latch fails under a life tick, 77 windows on one turn; T4 S6 own-loop verdict rendered 0× in 42 logs despite CV's rendered-bytes pin (own_loop_windows_asked 2); T5 AT BEST false under trample/menace (two seats) + gang-block spill omitted + self-contradicting header; T6 crack-back cover subtracts from the creature-only floor while the header prints the added total; T7 tap forecast false under Intruder Alarm; T8 held counterspell reads as not-in-hand (lost a game) and keep-X names Path to Exile with counters in hand; T9 sacrifice rows never price the spent blocker; ATTACK TOTAL doesn't fold Lightmine Field.
- LESSON OF WAVE 78 (in the wave79 brief): corpus-derived fixtures through the live caller + a predicted NON-ZERO counter; key stability proven only on the corpus's own re-asked pairs.
- Lanes CZ (window/hold: T2/T3/T4/T13/T12/T14), DA (combat truth: T5/T6/T7/T9/T16), DB (render/bytes/tools: T8/T10/T11/T15/T17 + T1 measure) launched 22:35 in worktrees/lanes/w79-{CZ,DA,DB} from 06fad1449; done-files W79_<L>_DONE; DC = Astra fix lane after gate 1.
- Tool note for the late check: the seat log's `turn` field (62/81) is not results.tsv's turn (85/68) — T17.

## 2026-09-11 23:40 — wave 79 step one merged (CZ, DB, DA); gate 1 + Astra running
- Merges: CZ `d091dd29a` clean; DB `f18c1dd1f` (ctor-init auto "(ci)"); DA `244d8b5f8` (one auto "(d)" hunk + ONE SEMANTIC SEAM: DA's `[#W79-DA] KEY` pin searched the key tail for the mixed-case action string while CZ's T3 now reduces every numbered row through `holdActionKeyRow`, which lower-cases — the pin's expectation was re-derived to the action key's spelling; flagged to Astra as a seam to verify). PARSETEST 6810/0 = 6642 + 46 + 56 + 66 exact. Suite from DA: 1293 (two new trample fixtures).
- Gate 1 unit w79-gate1; Astra prompt ~/.gatelogs/w79-review-prompt.md (attacks: T3's window-scope key replacing the board half — same turn+phase, same action keys, DIFFERENT legal option sets replayed; the collapse across phases; the DP's coverage; the trim's cut history). DC worktree seeded from 244d8b5f8.

## 2026-09-12 00:10 — wave 79 step one CLOSED; corpus launched
- DC (wave79/lane-DC.md) CONFIRMED all 11: F1 key board half = seam scope + legal-continuation digest (live canTarget target identities per row + untapped-source count; corpus pairs still equal, second Doom Blade target / X 3→4 unequal; deadlock breaker reads the board); F2 THREATENED marker carries the threat identity; F3 DP models both damage steps — two suite fixtures prove it in the engine's damage code (6/6 double-strike trampler vs 1/5 wall = 7; 6/2 first-striker kills a 6/6 trampler for 0); F4/F5 no invented life figures; F6 Ertai rejected; F7 Chrome Mox keeps the imprint clause; F8 Bell-Ringer not an untap source; F9 rank shift = 0 new; F10 face/count applied at the send; F11 the 1016−974 gap = 42 = one per seat log (narration offset).
- Merge DC `b1e1db7be` clean. Gate 2 (make -B): 1295/0, 76 AI/0, PARSETEST 6883/0. Archive `archives/wagic-5af6f7a95-w79step1` (ldd clean). Pushed. Worktrees w79-{CZ,DA,DB,DC} removed. Pilot idle, probe OK.
- Corpus: unit `w79-corpus` 00:07 CDT, outdir `~/.Wagic/ai/gpt/selfplay-runs/matchups-20260912-000748`, 21 concurrent, thinking ON. Watches early/terminal/late armed.
- Predictions to adjudicate at step 3 (non-zero or FAIL): `stop_reached_reputs_collapsed` 20-35; same-phase byte-identical re-asks → 0 and `ask_replays_reserved` +≥135; `own_loop_verdict_lines_rendered` ≥ 2 (needs an own-loop window); `ask_key_continuation_differs` > 0; `hold_reopened_new_threat` > 0; `hold_verdict_safer_ignored` > 0; `answer_label_absent_heuristic_played` on deck125 (guide-cause test); 0 AT BEST figures beaten by a legal assignment; deck130 max prompt ≤ ~22 KB.

## 2026-09-12 01:05 — FOUR OWNER RULINGS (verbatim), applied the same turn
1. On "S1 — may the parser read an unlabelled `<n> (<name>)` answer line after PLAN:" — "no such rule exists. Explicitly the rule is to 'make the parser robust'. And that 'any plan, must precede action.' Mechanically, for an llm, a plan that follows a choice is post hoc justification instead of load bearing reasoning." → the loop had INVENTED the "no parser reads outside the two labelled parts" rule (first written into a brief around wave 75 from the protocol-text ruling). Parser robustness is loop-owned. Lane DD launched now (worktree from b1e1db7be) to read the label-less line; merge after the wave-79 corpus. Swept: skill invariant 000(b)/(c), memory feedback_wagicgpt_reasoning_channel_only, wave79 brief RULINGS line, wave78 §T T1 (SUPERSEDED).
2. On the pilot `num_speculative_tokens` 4→1 / `--max-num-seqs` proposal — "No." → declined; recorded in memory spark_qwen36_mtp_crash; never re-propose; recovery recipe only.
3. On the lategame specimens — "Stop producing these. I no longer read them anyways. If i want one, ill ask for it." → invariant 0 RETIRED in the skill; the engine-seat brief item (5) is removed from the wave-79 brief onward; wave78/lategame-specimen.txt is the last.
4. On A333/A334 — "No, this skill is managed by the loop. I've reviewed and directed revisions only one time ever, when it was apparent by the sub agent behavior that there was an issue in the context they were receiving." → folded into wave68/strategy-writing-skill-v2.md §4.1 now (amendments file marked FOLDED); skill invariant 0a records the ownership; future amendments are folded by the loop the wave they are written.

## 2026-09-12 05:35 — wave 79 corpus COMPLETE; DD merged; step 3 seats launched
- Corpus `matchups-20260912-000748-final`: 21/21 natural in 5 h 16 min, no incident. **0 ask-seam fallbacks** (first since wave 76; 1 engine-answered reveal). `answer_label_absent` 0 — deck125's guide edit 4 removed the label-less habit (the guide was the cause); DD's parser fix is right by ruling but untested by this corpus.
- Census (~/.gatelogs/w79-stats.out, CENSUS CHECK RECONCILES, TURN SOURCES gap 42 = one per seat log): protocol_replies 2,203 (was 2,629); latency p50 94 s (was 78); `stop_reached_reputs_collapsed` 5 / skipped 8 (was 0/44); `ask_key_continuation_differs` 118; `identical_ask_answers_reserved` 0 (was 953) vs `ask_replays_reserved` 1,084 — engine seat to explain; `hold_verdict_safer_ignored` 59; `hold_reopened_new_threat` 0 (DC F2 prediction — FAIL unless the state never arose); `own_loop_verdict_lines_rendered` 63 / windows 4; `crossphase_board_unchanged` 26 (was 4); `async_drops` 15 (was 46). Wins 152 5/6, 162/146/130 4/6, 126 3/6, 125 1/6, 123 0/6 (deck123 4→0, deck130 0→4).
- DD merged `5bc856d84` clean (PARSETEST 6902/0); gate 3 unit w79-gate3 running (make -B); worktree removed. Brief wave79/reviewer-brief.md `c00eeda5d` (owner rulings in the head; specimen item RETIRED; regression watch (a)-(n) incl. the non-zero prediction adjudication). Eight seats launched 05:35.

## 2026-09-12 06:40 — wave 79 CLOSED (step 3); wave 80 step one launched
- Eight seats done: EDIT 146/152/125/130/123, KEEP 162/126; guides installed 36ae77199; no proposals; engine seat (45 PASS / 8 FAIL / 7 UNTESTED). Gate 3 (DD merged) green 1295/0, 76 AI/0, PARSETEST 6902/0; archive `archives/wagic-500bc49ba-w79step1b` = 5bc856d84 — the wave-80 corpus candidate binary unless step one changes it (it will: gate 2 make -B of the new master).
- §U HIGHs (wave79/known-bugs.md, commit 70345356d, pushed): U1 the X-payment loss is LIVE and caused by the plan echo trimming the cast clause before the completing menu (deck130/deck125 losses); U2 hold-latch counters unauditable (no record field; verdict rows render 0×) — DC F2 UNTESTED by default; U3 cover credits bodies the row doesn't leave; U4 split tap bill; U5 casting-menu format example from row 1 (62/139 picks); U6 cycling prices draw only; U7 one-sided `board sweep:` marker; U8 cross-phase dead lists at full price (26).
- LESSON OF WAVE 79 in the wave80 brief: every counter ships with a per-record trace and increments at the send; key-only verdicts are invisible (again); fix the emitter the seat reads.
- Lanes DE (windows/holds/keys), DF (combat/cover/pricing), DG (plan echo/menus/render) launched 06:40 in worktrees/lanes/w80-{DE,DF,DG} from 70345356d; DH = Astra fix lane after gate 1.

## 2026-09-12 08:20 — wave 80 step one merged (DE, DF, DG); gate 1 + Astra running
- Merges: DE `bca035b57` clean; DF `2c6b176a2` (auto f + d); DG `a66df551f` (auto f + d). PARSETEST 7035/0 = 6902 + 32 + 55 + 46 exact. Suite from DF 1296 (one new cycling fixture: engine charges 3 where the row said 1).
- Lane notes: DE — U2 verdict rows rendered + record fields (re-open yield NOT widened: giving crack-back/stack-death an identity would put a rowless board fact into the latch key; U2 discharged as auditability); U8 replay requires byte-identical rows + not entering a sorcery-speed window (25 of wave 79's 26 admitted); U9 the 22 residual pairs = the `sources=` digest term working as designed; U10 the 7 `none` writes were LAND-DROP menus consuming a face staged by an in-flight cast window. DF — U3c spent-blocker clause renders 0 on this corpus (all 60 rows sacrifice a Clue/fetch or the give-back is 0) — UNTESTED live; the Silverquill "dead mode credited" claim does NOT reproduce; U13 engine-answers one-outcome menus with a `menu_single_outcome` record. DG — U1 three-state cast-step latch (pure-pinned, never live-driven); U5 exemplar was `askExemplar` render text, not golden protocol; U11 used a factual own-spell marker (not "never a counter target"); the roster under-report half NOT done (cited roster was complete); U16 fold −91 B/prompt only (hold ROW fold reverted — C14 pins); U17 answer: latency r=+0.75 vs reasoning_chars, −0.30 vs bytes — the 78→94 s rise is reasoning output.
- Gate 1 unit w80-gate1 launched; Astra prompt ~/.gatelogs/w80-review-prompt.md; DH worktree seeded from a66df551f.

## 2026-09-12 09:20 — wave 80 Astra: 12 findings; MERGE DAMAGE (F1) repaired; DH launched
- Gate 1 on a66df551f: 1296/0, 76 AI/0, PARSETEST 7035/0.
- ⚠ F1 CONFIRMED: `merge-lane.sh`'s auto-resolve patterns (f)/(d) on the ctor-initializer hunk took ONE side twice — nine members (DE 7: verdict-line counters/seqs, cross-phase replay, replay split; DF 2: single-outcome counters) were left UNINITIALISED; `check-ctor-init` only catches commented-out initializers. Repaired by hand (d70c14fac; PARSETEST unchanged 7035/0 — the pins never read the members before a write). NEW PROCEDURE (replaces "trust the helper's (ci)/(d)/(f) resolves"): after EVERY merge-lane.sh run, `python3 ~/.gatelogs/check-merge-inits.py <lane-branches...>` from the repo root — exit 1 = missing initializers, restore by union before the trailing comment. DH adds the header-vs-ctor guard at link time.
- Other findings: F2 HIGH an answer BEFORE the plan is still accepted (inherited; owner's "any plan, must precede action" — DH rejects it); F3 HIGH a hold survives a second lethal stack (rank clamp; fix = threat IDENTITY event); F4 HIGH rendered crack-back verdict ignores blockers; F5 HIGH U1's latch retires at the first of several completing menus; F7 HIGH activated self-bounce read as ETB; F8 HIGH "absorbs full power" false for trample; F6/F9/F10/F11/F12 MED. Brief section "Lane DH" (288556656); DH launched 09:20.

## 2026-09-12 10:05 — wave 80 DH merged; gate 2 running
- DH (wave80/lane-DH.md) CONFIRMED all 12: F3 lethal face = EVENT keyed on object identity (internal only, no key/row/prompt); F4 rendered crack-back verdict runs the blockers DP and names its floor; F5 cast step closes on the cast EVENT; F2 answer-before-plan REJECTED (population empty for 3 waves); F7/F8 ETB-trigger gate + trample-aware absorb sentence; F10 send-time faces/counters inside askModel via a scoped guard (destructor); F11 per-event `hold_event` records reconciled by corpus-stats --selftest; F12 record-seq join; F1 guard: check-ctor-init fails on a header scalar neither initialised nor body-assigned (26 AIPlayerGPT scalars are body-assigned). FOUND, NOT FIXED — DOCKET wave 81: 55 genuinely uninitialised scalars in SimpleButton/SimplePad/SimplePopup/TestSuiteAI/TextScroller etc. (counted on every build, enforced for nothing).
- Merge DH `cf72a002f` clean; check-merge-inits ok for all four lanes; PARSETEST 7086/0. Gate 2 unit w80-gate2 launched.

## 2026-09-12 07:45 (Voyager clock; earlier 2026-09-12 stamps in this file after 05:35 were estimated and run ~2.5 h fast) — wave 80 step one CLOSED; corpus launched
- Gate 2 (make -B on cf72a002f): 1296/0, 76 AI/0, PARSETEST 7086/0. Archive `archives/wagic-a7a6b8942-w80step1` (ldd clean). Pushed. Worktrees w80-{DE,DF,DG,DH} removed. Pilot idle, probe OK.
- Corpus: unit `w80-corpus`, outdir `~/.Wagic/ai/gpt/selfplay-runs/matchups-20260912-074153`, 21 concurrent, thinking ON. Watches early/terminal/late armed.
- Predictions to adjudicate at step 3 (non-zero or FAIL, each with its record trace): `dropped after its X was announced` 0 and 0 leading-clause losses at completing menus (U1+F5); `hold_event` records > 0 with `hold_reopen_reason` incl. `new_lethal_stack` when the state arises (U2/F3/F11); `crossphase_replayed` 10-40 traced in askreplay/ (U8); `crack-back cover: NONE from this body` > 0 (U3a); `menu_single_outcome` records > 0 (U13); verdict lines rendered (crack-back / stack-death) > 0 and never on a suppressed window (U2/F10); `action_before_plan_replies` (F2) — 0 expected unless the shape appears; the robust parser's first live test (DD); deck146 bytes −91 B/prompt; row-1 pick rate at the ask seam falls (U5).

## 2026-09-12 13:50 — wave 80 corpus: 20/21 natural + 1 HUNG (engine dead loop); rerun + hot-fix lane DI launched
- `matchups-20260912-074153`: 20 natural finishes; `162v123` (epoch 1789216936/1789216939) HUNG — the harness killed it at 3641 s with no seat record (dead loop, not a slow decision). Evidence: deck123 seat log seq 32 (turn 8 Main 1): the model answered `CHOICE: 1 (Create human with Thraben Doomsayer)` with PLAN "Tap Thraben Doomsayer for a Human token. Next turn…" — ONE token intended; stderr then shows 3,674 `Using Activated ability` on Doomsayer, each followed by `AIPlayerGPT: the menu is unchanged after action N but the board moved - that activation progressed the game, so priority is not force-passed` and `take action N/N (cached=N)`. Intruder Alarm untaps the Doomsayer after every token, so the row bytes, the legal-continuation digest (target ids + untapped-source count) and the option set never change; the cached answer is replayed without bound; the model is never re-asked; no stop was stated. Two wave-79/80 changes combined: DC F1's deadlock breaker ("board moved → not force-passed") + the ask-cache replay on an identical window. 6,223 `cast ask NOT issued` lines are the same loop's casting windows.
- Rerun launched (unit w80-rerun, pool `matchups-20260912-074153-rerun-01-162v123`, SAME binary a7a6b8942) — if it hangs again the fixed binary reruns it (mixed corpus, stated in the brief).
- Hot-fix lane DI (worktree w81-DI from cf72a002f): a cached answer that activates/casts is replayed at most ONCE per window; the second identical offer after a board change RE-ASKS (the model restates its plan/stop); the digest for object-creating rows includes the seat's battlefield object count; pin on the corpus records; a suite fixture Doomsayer+Intruder Alarm must terminate.

## 2026-09-12 14:05 — rerun KILLED by a false REGIME GATE (harness scans the shared logs dir); lane DJ launched
- `w80-rerun` (162v123, same binary): its own 2 seats PASSED the regime gate (10 records), then the gate re-checked and found `1789239545-ai_baka_deck198-…-vs-ai_baka_deck199.jsonl` — the DI hot-fix lane's suite-fixture games (GPT seats, thinking OFF, stub endpoint) landing in the SHARED `~/.Wagic/ai/gpt/logs` dir — and killed the run as "20 of 20 checked records thinking=off". The regime gate keys on the shared dir by recency, not on the run's manifest (the harvest has been manifest-keyed since CU F10; the gate was not). Same family as the wave-76 harvest-by-mtime defect.
- Lane DJ (worktree w81-DJ from cf72a002f): the regime gate (and every other harness check that reads seat logs mid-run) keys on the run's OWN announced seat-log names, never on the shared dir by time; --selftest gains a "foreign thinking-off log in the shared dir during the run" case that must PASS the gate.
- The 162v123 rerun waits for DI + DJ merged and gated; it then runs on the NEW binary (MIXED corpus: 20 on a7a6b8942 + 1 on the step-1b binary — the brief must say so).

## 2026-09-12 15:00 — DJ + DI merged; gate 3 running
- DJ `76e4e1a18` (tools-only): six shared-dir scan sites (regime-gate run_records, pilot_stall_verdict, window_loop_verdict, no_progress_sweep, the cap adjudicator, the feasibility projection) now key on the run's manifest (`$OUTDIR/.seatlogs` + live WAGIC_GPT_TRANSLOG_FILE announcements, per game) via new tools/runmanifest.py; selftests harvest 16, pilot-stall 21, window-loop 14, regime 34, 0 failed. Weakest: manifest-less outdirs degrade to the old epoch scan.
- DI `337becbf2`: the replay path was `chooseOrderedAction`: `unchanged = (askKey == mLastAskKey) → choice = mLastChoice` with NO bound (the ask/cast seam has had askReplayRefuseScoped since W71). Fix: an activating/casting cached answer stands at most once per key then re-asks (latched across kChoicePending ticks); the digest gains the seat's battlefield count only when an offered row creates/moves objects (ATokenCreator/AAMover/AACopier), pinned out of hold/option/re-put keys; a replay at/past the stated stop is refused; counter `cached_replay_reasked` + record `reask_reason` at the send. RED/GREEN by a live probe (`tools/w81-di-alarm-loop-probe.sh`, dev-build-only `WAGIC_GPT_STUB` under `_DEBUG || WAGIC_DEVLOGS`): base 916 activations / exit 124 never ended; fixed 226 activations, winner at turn 20. DEVIATION: a suite fixture cannot reach AIPlayerGPT (TestSuiteAI is Baka) — probe instead. PARSETEST 7102/0; suite 1296/0 + 76 AI/0.
- Gate 3 (unit w80-gate3, make -B on 337becbf2) launched; on green: archive `-w80step1b`, push, rerun 162v123 on it.

## 2026-09-12 14:35 — gate 3 green; rerun attempt 2 on the fixed binary
- Gate 3 (make -B on 337becbf2): 1296/0, 76 AI/0, PARSETEST 7102/0. Archive `archives/wagic-ba5cdc374-w80step1b`. Pushed.
- Rerun attempt 2: unit w80-rerun3, outdir `matchups-20260912-074153-rerun-02-162v123`, binary ba5cdc374 (MIXED corpus with the 20 games on a7a6b8942 — the brief must say so). Attempt 1's dir kept as `…-rerun-01-162v123-attempt1-killed` (not a corpus). Two procedural slips on the way: a sed on `-rerun-01-162v123` missed the `$R-01-162v123` variable form (the relaunch briefly reused attempt 1's outdir — stopped within seconds, nothing written); `pkill -f '<pattern>'` killed the invoking shell because the pattern matched its own command line — use `pgrep -f '[s]elfplay…'` bracket patterns.

## 2026-09-12 15:45 — wave 80 corpus CONSOLIDATED (mixed binary); step 3 seats launched
- `matchups-20260912-074153-final`: 20 games on a7a6b8942 + `162v123` rerun on ba5cdc374 (attempt 2, natural, deck162 won); 42/42 gameend, census reconciles (kind sum 2,347 incl. hold_event 180, menu_single_outcome 3). Attempt 0's stderr kept in the dir as `…-HUNG-attempt0.stderr`; attempt 1's dir `…-rerun-01-162v123-attempt1-killed` is not a corpus.
- Census highlights: 0 ask-seam fallbacks; 1 priority/unparsed_reply (`prose_outside_two_lines` — the robust parser's first live test, engine seat quotes it); latency p50 87 s; plan_line_missing 2.9%; `hold_events` 180 = safer_ignored 71 + new_lethal 7 + new_threat 0 (+102 other reasons — engine seat reconciles); crack-back verdict lines 382 (26 lethal-blocked-away), stack-death 36; `crossphase_replayed` 4 (predicted 10-40) with board_unchanged 0; `plan_cast_steps_closed` 376; `dropped after its X was announced` 2 in the dir's stderr (one may be attempt 0's — engine seat attributes); `menu_single_outcome` 3 / rows spared 60; `cached_replay_reasked` 0 (rerun binary only); stop-reached chains 1. Wins 152/130 4/6, 126/125/162/146 3/6, 123 1/6.
- Brief wave80/reviewer-brief.md bd3c55f50 (MIXED binary stated; regression watch (a)-(n); no specimen). Eight seats launched 15:45.

## 2026-09-12 16:02 — wave 80 step 3 CLOSED; wave 81 lanes DK/DL/DM launched
- Eight seats reported. Guides installed: 125 (19,957 B, 5 edits — both stranded-X drops were this deck's X-menu Decline), 152 (19,918), 123 (19,952), 146 (19,995), 162 (19,971), 130 (19,832); 126 KEEP. All pass size/cites/YOUR PLAN/U+FFFD/check-reply-instructions. Commit `8e6554ef7`.
- `wave80/known-bugs.md` §V (18 items) + `wave81/lane-brief.md` (LESSON OF WAVE 80 ×4; lanes DK holds/response/send-boundary = V1 V3 V4 V9 V15; DL verdict truth = V5 V7 V8 V10 V13-spare V17-gap; DM engine/echo/intel/bytes = V6 Exquisite Blood trigger FIRST, V11 V12 V13 V14 V16 V17 V2-residual; DN = Astra fix lane). Commit `4df71b19b`, pushed.
- Engine seat scoreboard: DE 9/2/1, DF 11/1/3, DG 8/4/1, DH 8/5/4, DI 3/0/2 (UNTESTED — the Alarm board never formed in the rerun), DJ 2/0/0. `dropped after its X was announced` = 2, BOTH corpus games (my earlier attribution to the HUNG stderr was wrong). Bytes regressed +9.1% mean, >20 KB 5.6→13.4%.
- Top new engine findings: V3 `new_lethal` re-open is a logged no-op (`125v162` seqs 18-24, seat died un-asked); V4 an instant cannot be cast in response after the phase's casting decision (250 prompts, 61 with a live stack — the corpus's only unparsed reply); V5 crack-back verdict contradicts its own ADD THOSE UP total on 11 windows; V6 Exquisite Blood never triggered on combat damage (`130v126`, engine rules bug candidate); V7 own-loop starter wrong 18/18 (block triggers with no enemy creatures).
- Worktrees `worktrees/lanes/w81-{DK,DL,DM}` seeded from 4df71b19b; done-files `W81_{DK,DL,DM}_DONE` cleared. Baselines for the lanes: suite 1296/0 + 76 AI/0, PARSETEST 7102/0.
- Next: merge DK/DL/DM (`merge-lane.sh` + `check-merge-inits.py`), gate 1, Astra review (diff 4df71b19b..HEAD), DN fix lane, gate 2, archive `-w81step1`, corpus.

## 2026-09-15 01:30 — ENGINE AUDIT CLOSED; master accepted at badbba7d3
- Owner stopped the loop 2026-09-12 ("the code is getting royally screwed"). Audit: Astra + Fable read the 09-03..HEAD rules-engine delta against the official CR; heuristic-only A/B (tools/baka-ab.sh) + per-commit sweeps localised every outcome shift to waves 44-57 (126v152 = 3329c40f1; 123 = a drift over the wave-51/52 Baka changes) and found a 14-commit 10% hang (d7526c7bf -> 0026a9d87). Full record: strategy-design/audit-2026-09/.
- The new gate caught two live heuristic crash classes on master on its first run (selectMenuOption indexing mObjects with a control id; MayAbility::Update on a dead card). Four fix passes, each adversarially read (Astra x2, Fable x2 after the Codex pool ran out), each merged only on a clean 630-game gate. Accepted: badbba7d3 (suite 1309/0 + 80 AI/0, PARSETEST 7275/0).
- Procedure changes (in the skill, synced to Spark): HEURISTIC A/B GATE before every shipping binary; the loop RESET to the owner's three steps + rotation every wave + the DROPPED-inventions list.
- Next step one = audit-2026-09/bug-list.md. No corpus has run since wave 80; wave 81's DK/DL/DM are merged.

## 2026-09-15 11:35 — bug-list lane + PARSETEST split merged; accepted 62a5733ec
- GUI check PASS (human edict chooser observed twice; edict-gui-check.md). Census: PARSETEST was 40.6% of AIPlayerGPT.cpp; split to AIPlayerGPTSelfTest.cpp, byte-identical output, 106,056 -> 63,671 lines (split-parsetest.md). Bug-list items 2/3/4/5/8 closed (buglist-lane.md) — the heuristic A/B gate caught a wrong mAbilityEpoch init that suite+PARSETEST missed. Accepted binary 62a5733ec (1312/0 + 81 AI/0, PARSETEST 7275/0, 630 games clean). Worktrees cleaned. Open: bug-list #11/#12 + watch items. Loop resumes at step one / rotation / corpus.

## 2026-09-15 22:30 — GPT-LAYER AUDIT: cleanup merged (6fc6f5947, gate running)
- Owner rulings today (verbatim): "Audit the gpt layer. Clean up crud, simplify the needlessly complex, fix flaws. list apparent potential improvements for my review, if any of those exist." / cross-review idea: "can have them review eachothers work? Possible that they will agree on findings with no overlap." / 16:10 "Wait for 7pm. Have astra do it. I want the extra pov explicitly." / 17:55 "Dont throw away work wtf." (I had stopped the cleanup lane and deleted its branch on a misread; restored from reflog, no loss but one uncommitted edit and a probe rerun).
- Two first-pass reviews (Astra, Fable) + Fable cross of Astra + a Fable STAND-IN as Astra (Codex pool out) → LEDGER.md (provisional). Astra's own cross-review after the 19:00 reset refuted 3x more than the stand-in — the extra POV was real (counters, live emitters, parser tolerance, scenario labels, bottom order) → LEDGER-v2.md; the cleanup lane revised R1-R4 and caught two errors of its own that the stand-in would have passed (a forced-equal number = a new lie; a deleted counter that was also a record seq).
- Merged: eleven flaw families fixed (withholds/auto-answers removed, keys = board state + question, watchdog reachable, mutex fixed, one CrackBackFacts per window with scenario labels, one mana policy, retry legs slot-keyed, parser accepts the ruled shapes, twelve wrong-scope renders fixed, telemetry per window, dead cast-seam auto-pass gone), confirmed crud deleted, epitaphs 73→48. All on gpt-layer/cleanup-lane.md. Section D = owner's 13 proposals, untouched.

## 2026-09-17 13:10 — GPT-LAYER AUDIT COMPLETE; proposals merged (9c5c2058e, pushed)
...
## 2026-09-18 01:35 — wave-81 corpus COMPLETE (6 h 09 min, no incident); step three seats launched
- `~/.Wagic/ai/gpt/selfplay-runs/matchups-20260917-191141-final`: 21/21 natural finishes, 0 HUNG/crash/timeout, one binary (6e0c51238). Census (`~/.gatelogs/w81-corpus-stats.txt`) RECONCILES: 2,349 records = 2,122 round trips + 226 hold_event + 1 recovery; records are `kind: window` + `seam` (P11). ONE fallback corpus-wide (ask/unparsed_reply, deck50 seat vs 125). plan_line_missing 2.9% (flat), latency p50 97.9 s (wave 80: 87.0; reasoning median 5,770→6,089), `async_drops` 52 (wave 80: 9 — seats asked to explain), `cast_decision_reopened_new_stack` 189 (V4 live), 3 `order` windows (P10's first live firing), `own_loop_verdict_lines_rendered` 0. Wins 162 5/6, 152 5/6, 146 4/6, 130 3/6, 125 2/6, 50 1/6 (guideless), 123 1/6.
- Regime gate PASSED on the P11 schema at +33 min (bug-list #14 narrowed to the early-check reader; 9b27fdb9d). Pre-compaction Monitors survived compaction (verified by their notifications).
- Step three: `wave81/reviewer-brief.md` (per-deck seats only + synthesis; P9 land-row and P5 narration counts requested from every seat; deck50 seat authors the first Erosion guide). Seven Opus seats launched 01:30.

## 2026-09-18 02:45 — wave 81 step three CLOSED (35e1e82dc); wave-82 step one launched (3 Fable lanes)
- Seats: 123 KEEP, 162 KEEP, 130 EDIT (3), 146 EDIT (1), 152 EDIT (5), 125 EDIT (2 rules), deck50 first guide AUTHORED (14,671 B). All installed under bin/Res (38 guides pass check-reply-instructions). Synthesis: R332 (own-library deck-out + mill-on-cast) and R333 (counter test keyed to printed facts) adopted into wave81/general-strategy.md (not loaded at runtime); no skill amendment; merged list 10 HIGH / 14 MED / 7 LOW.
- P9 (owner's revert call, counts only): ~388 land-row windows, 7 declined (~6 turns), 0 land-ahead-of-spell at every seat — measured on a HALF-SHIPPED regime: the legacy "the land drop is its OWN decision" header still prints on every folded menu (H1; one traced lost drop, deck152 vs 130 seq26) and the standalone land ask still fires when no casting menu exists (H2). Lane EB fixes H1/H2 and tags `land_shape` so the next corpus measures one regime.
- Two ruling violations found by seats: the reasoning-budget retry re-asks with thinking OFF (H6 → lane EA); the corpus's one fallback `PLAN: 4 (Cast nothing right now)` was an unambiguous answer the robust parser should have read, and fell to Baka with 891 s left instead of a re-ask (H7 → lane EA).
- Worktrees worktrees/lanes/w82-{EA,EB,EC} from 35e1e82dc; done-files W82_E{A,B,C}_DONE. Baselines: suite 1314/82 AI (flake intrepid_adversary_repeated_payment), PARSETEST 7314/0.

## 2026-09-24 01:30 — wave-82 step one MERGED (05383b8f4); gate 1 running
- The three Fable lanes were cut off by the Fable limit on 09-18; commits survived (EA 4/6, EB 9/9 + lane file, EC 9/9). Resumed 09-24 after the owner's "Reset happened. Continue. Do not start a new corpus, we'll use that as a seam for ending the session."
- INCIDENT (mine): EB/EC suite units ran SIX DAYS with 0 tests — I seeded the lane worktrees without the untracked `bin/User/` profile; without `User/settings/options.txt` wagic sits in first-run setup forever (log ends after `inserted item ptr`, main thread idle in SdlApp::OnExecute). The skill already said to copy bin/User; the line now names the file and excludes the 12 GB `User/sets/` (my first rsync copied 20 GB of card art into two worktrees before I caught it; removed). Isolation: master binary in the worktree dir stalls, lane binary in the main dir runs — the DIRECTORY, not the code.
- Lanes gated: EA PARSETEST 7332/0, suite 1314 (1 = known flake)/82; EB 7370/0, suite 1396/0/0; EC 7385/0, suite 1314/82 0 failed. Merges: EB clean; EC needed one hand fix (the resolver's PARSETEST tail union left a stray 8-space `}` at the seam → "expected unqualified-id before '{'" ×N; deleted, 7441/0); EA one hunk auto-resolved (ctor line), 7459/0 = exactly additive (7314+56+71+18). check-merge-inits flags `mAsyncDropStampsSeq` MISSING for EB/EC — EXPLAINED: EA's M3 (0843ff9bd) deliberately removed that base member (the L10 stamp reached 0 of 52 drops), so EB/EC's untouched ctor lists still name it; not damage. check-ctor-init OK (124 files).
- EA highlights: H7 re-ask recovers 65/74 unreadable replies live; H6 retry keeps thinking on (117 retry records `thinking: on`); #13 paired proof 19/20 seeds trace-identical (the 20th bimodal on its own seed — Baka address-order residual). EB: M4/M5 were ALREADY TRUE — reviewers read `options_text` (the answer key) as rendered rows → next reviewer brief must say so. EC: H3/H5 numbers pinned outside every key.
- Gate 1 (unit w82-gate1: make -B, suite THREADS=1, PARSETEST) running; then baka-ab 10x4 vs 62a5733ec; archive `-w82step1`; rotation. NO corpus this session (owner).

## 2026-09-24 01:50 — wave-82 step one ACCEPTED (05383b8f4); SESSION ENDS at the corpus seam (owner)
- Gate 1: make -B, suite 1395 ok / 1 failed (the known intrepid_adversary flake) / 0 timed out, PARSETEST 7459/0. Unpaired A/B 10x4 vs 62a5733ec: PASS hard, flags 125 +2.50, 130 −2.11, 152 −2.02.
- First paired gate: run 1 was VOID (the 62a5733ec control predates the seed hook — `strings` shows no WAGIC_SELFPLAY_SEED — so 88/210 "discordant" was clock-vs-seed noise; bug-list #15). Run 2 with the proper control (6e0c51238 + cherry-picked ac4598bd9 = base + seed hook only, branch w82-base-seed): **210/210 concordant, 209 on the same turn, 0 unfinished** — this wave moved heuristic play by exactly nothing. Archived: `archives/wagic-05383b8f4-w82step1`, `archives/baka-ab/05383b8f4` (unpaired), `archives/baka-ab/05383b8f4-paired` (the seeded candidate run = the paired baseline for the next gate: `baka-ab.sh --paired=archives/baka-ab/05383b8f4-paired <bin> <out> 10 4` then `compare.py --paired`).
- Rotation: no deck eligible (wave82/rotation.md). Corpus launch script STAGED, NOT RUN (owner: "Do not start a new corpus, we'll use that as a seam for ending the session"): `~/.gatelogs/w82-corpus-launch.sh` — next session: `cmp projects/mtg/bin/wagic archives/wagic-05383b8f4-w82step1` (relink if boost moved), `ldd | grep 'not found'`, Spark 8084 up, `rm -f ~/.gatelogs/W82_CORPUS_DONE`, `systemd-run --user --unit=w82-corpus -p MemoryMax=12G -p MemorySwapMax=0 ~/.gatelogs/w82-corpus-launch.sh`, arm terminal grep + one late check (+8 h).
- Lane worktrees removed (branches w82-lane-E{A,B,C}, w82-base-seed kept). master d8bd44bfb pushed.
